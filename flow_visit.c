...
void flow_visit_declaration(struct flow_visit_ctx *ctx,
                            struct declaration *p_declaration);
static void flow_visit_attribute_specifier_sequence(
    struct flow_visit_ctx *ctx,
    struct attribute_specifier_sequence *p_visit_attribute_specifier_sequence);

static void
flow_visit_secondary_block(struct flow_visit_ctx *ctx,
                           struct secondary_block *p_secondary_block);
static void flow_visit_struct_or_union_specifier(
    struct flow_visit_ctx *ctx,
    struct struct_or_union_specifier *p_struct_or_union_specifier);
static void flow_visit_expression(struct flow_visit_ctx *ctx,
                                  struct expression *p_expression);
static void flow_visit_statement(struct flow_visit_ctx *ctx,
                                 struct statement *p_statement);
static void flow_visit_enum_specifier(struct flow_visit_ctx *ctx,
                                      struct enum_specifier *p_enum_specifier);
static void flow_visit_type_specifier(struct flow_visit_ctx *ctx,
...
static struct object make_object_core(struct type *p_type,
        ...
        if (p_member_declaration->member_declarator_list_opt) {
          ...
          while (p_member_declarator) {
            if (p_member_declarator->declarator) {
              char *tag = NULL;
              if (p_member_declarator->declarator->type
                      .struct_or_union_specifier) {
                tag = p_member_declarator->declarator->type
                          .struct_or_union_specifier->tag_name;
              } else if (p_member_declarator->declarator->type.next &&
                         p_member_declarator->declarator->type.next
                             ->struct_or_union_specifier) {
                tag = p_member_declarator->declarator->type.next
                          ->struct_or_union_specifier->tag_name;
              }
              ...
              if (tag && has_name(tag, &l)) {
                ...
                struct object member_obj =
                    make_object_core(&p_member_declarator->declarator->type, &l,
                                     p_deep, declarator);
                objects_push_back(&obj.members, &member_obj);
              }
...
static void print_object(int ident, struct type *p_type,
                         struct object *p_object, const char *previous_names) {
  if (p_object == NULL) {
    return;
  }
    ...
    if (p_struct_or_union_specifier) {
      ...
      while (p_member_declaration) {
        ...
        if (p_member_declaration->member_declarator_list_opt) {
          ...
          while (p_member_declarator) {
            if (p_member_declarator->declarator) {
              char *name = p_member_declarator->declarator->name
                               ? p_member_declarator->declarator->name->lexeme
                               : "";
              char buffer[200] = {0};
...
static void set_object(struct type *p_type, struct object *p_object,
          ...
          while (p_member_declarator) {
            if (p_member_declarator->declarator) {
              if (member_index < p_object->members.size) {
                set_object(&p_member_declarator->declarator->type,
                           &p_object->members.data[member_index], flags);
              } else {
                // TODO BUG union?
              }
              member_index++;
...
void visit_object(struct parser_ctx *ctx, struct type *p_type,
                  struct object *p_object, struct token *position_token,
  ...
  if (!(p_type->type_qualifier_flags &
        (TYPE_QUALIFIER_OWNER | TYPE_QUALIFIER_OBJ_OWNER))) {
    return;
  }
  ...
  if (p_type->struct_or_union_specifier && p_object->members.size > 0) {
        ...
        while (p_member_declarator) {
          ...
          if (p_member_declarator->declarator) {
            ...
            visit_object(ctx, &p_member_declarator->declarator->type,
                         &p_object->members.data[member_index], position_token,
                         buffer);
    ...
    if (type_is_pointer(p_type)) {
      if (p_object->state == OBJECT_STATE_UNKNOWN ||
        ...
        if (t2.type_qualifier_flags & TYPE_QUALIFIER_OWNER) {
          visit_object(ctx, &t2, p_object->pointed, position_token, buffer);
        }
        type_destroy(&t2);
      }
    }
    ...
    if (!(p_type->type_qualifier_flags & TYPE_QUALIFIER_OBJ_OWNER) &&
      ...
      compiler_set_error_with_token(
          C_DESTRUCTOR_MUST_BE_CALLED_BEFORE_END_OF_SCOPE, ctx,
          p_object->declarator->name, "object '%s' was not moved/destroyed",
          name);

      if (p_object->declarator)
        compiler_set_info_with_token(W_NONE, ctx, position_token,
                                     "end of '%s' scope", name);
    }
  }
}
...
static bool check_defer_and_variables(struct flow_visit_ctx *ctx,
  ...
  while (deferchild != NULL) {
    if (deferchild->defer_statement) {
      flow_visit_secondary_block(ctx,
                                 deferchild->defer_statement->secondary_block);
      ...
      visit_object(ctx->ctx, &p_declarator->type, &p_declarator->object,
                   position_token, name);
    }
...
static bool flow_find_label_unlabeled_statement(
    struct flow_visit_ctx *ctx,
    struct unlabeled_statement *p_unlabeled_statement, const char *label);

static bool check_all_defer_until_try(struct flow_visit_ctx *ctx,
                                      struct flow_defer_scope *deferblock,
                                      struct token *position_token) {
  bool found_error = false;
...
static bool
flow_find_label_block_item_list(struct flow_visit_ctx *ctx,
                                struct block_item_list *p_block_item,
                                const char *label) {
  ...
  while (block_item) {
    if (block_item && block_item->label && block_item->label->name &&
      ...
      if (flow_find_label_unlabeled_statement(
              ctx, block_item->unlabeled_statement, label)) {
        return true;
      }
    }
...
static bool flow_find_label_statement(struct flow_visit_ctx *ctx,
...
static bool flow_find_label_unlabeled_statement(
  ...
  if (p_unlabeled_statement->primary_block) {
    if (p_unlabeled_statement->primary_block->compound_statement) {
      if (flow_find_label_block_item_list(
              ctx,
...
static bool flow_find_label_statement(struct flow_visit_ctx *ctx,
  ...
  if (statement->labeled_statement) {
    ...
    if (flow_find_label_unlabeled_statement(ctx, statement->unlabeled_statement,
                                            label))
      return true;
  }
  return false;
...
static bool flow_find_label_scope(struct flow_visit_ctx *ctx,
                                  struct flow_defer_scope *deferblock,
                                  const char *label) {
  if (deferblock->p_iteration_statement) {
    ...
    if (deferblock->p_selection_statement2->else_secondary_block_opt) {
      if (flow_find_label_statement(ctx,
                                    deferblock->p_selection_statement2
                                        ->else_secondary_block_opt->statement,
                                    label))
        return true;
    }
    ...
    if (deferblock->p_try_statement->secondary_block) {
      if (flow_find_label_statement(
              ctx, deferblock->p_try_statement->secondary_block->statement,
              label))
        return true;
    }
    ...
    if (flow_find_label_block_item_list(
            ctx, &deferblock->p_function_body->block_item_list, label)) {
      return true;
    }
  }
  return false;
...
static bool check_all_defer_until_label(struct flow_visit_ctx *ctx,
                                        struct flow_defer_scope *deferblock,
                                        const char *label,
                                        struct token *position_token) {
  ...
  while (p_defer != NULL) {
    if (!flow_find_label_scope(ctx, p_defer, label)) {
      error_found = error_found ||
                    check_defer_and_variables(ctx, p_defer, position_token);
    } else {
      break;
    }
    p_defer = p_defer->previous;
...
static bool check_all_defer_until_end(struct flow_visit_ctx *ctx,
                                      struct flow_defer_scope *deferblock,
                                      struct token *position_token) {
  ...
  while (p_defer != NULL) {
    found_found =
        found_found || check_defer_and_variables(ctx, p_defer, position_token);
    p_defer = p_defer->previous;
  }
  return found_found;
}
...
static void set_all_until_end(struct flow_visit_ctx *ctx,
                              struct flow_defer_scope *deferblock,
                              enum object_state state) {
...
static void
flow_visit_defer_statement(struct flow_visit_ctx *ctx,
...
static void flow_visit_try_statement(struct flow_visit_ctx *ctx,
  ...
  check_defer_and_variables(ctx, p_defer,
                            p_try_statement->secondary_block->last_token);
...
expression_is_comparing_owner_with_null(struct expression *p_expression) {
  ...
  if (p_expression->expression_type == PRIMARY_EXPRESSION_DECLARATOR) {
    ...
    if (p_expression->left->expression_type == PRIMARY_EXPRESSION_DECLARATOR) {
      if (type_is_pointer(&p_expression->left->type) &&
          p_expression->left->type.type_qualifier_flags &
...
static void flow_visit_selection_statement(
    struct flow_visit_ctx *ctx,
  ...
  */
  ...
  if (p_selection_statement->expression) {
    p_object = expression_is_comparing_owner_with_null(
        p_selection_statement->expression);
    if (p_object) {
      struct type type = {0};
      type_destroy(&type);
    }
  ...
  if (p_object) {
    if (state2 == OBJECT_STATE_UNINITIALIZED || state2 == OBJECT_STATE_MOVED) {
      p_object->state = OBJECT_STATE_NULL_OR_UNINITIALIZED;
    }
  }
}
...
static void
flow_visit_compound_statement(struct flow_visit_ctx *ctx,
                              struct compound_statement *p_compound_statement);

static void
flow_visit_initializer_list(struct flow_visit_ctx *ctx,
                            struct initializer_list *p_initializer_list);

static void flow_visit_bracket_initializer_list(
    struct flow_visit_ctx *ctx,
    struct braced_initializer *p_bracket_initializer_list) {
  if (p_bracket_initializer_list->initializer_list == NULL) {

  } else {
    flow_visit_initializer_list(ctx,
                                p_bracket_initializer_list->initializer_list);
  }
}

static void flow_visit_designation(struct flow_visit_ctx *ctx,
                                   struct designation *p_designation) {}

static void flow_visit_initializer(struct flow_visit_ctx *ctx,
...
static void flow_visit_type_qualifier(struct flow_visit_ctx *ctx,
                                      struct type_qualifier *p_type_qualifier) {

}

static void flow_visit_specifier_qualifier(
    struct flow_visit_ctx *ctx,
...
static void flow_visit_specifier_qualifier_list(
    struct flow_visit_ctx *ctx,
...
static void flow_visit_declarator(struct flow_visit_ctx *ctx,
                                  struct declarator *p_declarator);
static void flow_visit_type_name(struct flow_visit_ctx *ctx,
                                 struct type_name *p_type_name) {

  flow_visit_specifier_qualifier_list(
      ctx, p_type_name->specifier_qualifier_list, &p_type_name->type);
...
static void flow_visit_argument_expression_list(
    struct flow_visit_ctx *ctx,
...
static void
flow_visit_generic_selection(struct flow_visit_ctx *ctx,
...
static int compare_function_arguments2(
  ...
  while (p_current_argument && p_current_parameter_type) {
    if (p_current_parameter_type->type.type_qualifier_flags &
            TYPE_QUALIFIER_OWNER ||
        p_current_parameter_type->type.type_qualifier_flags &
            TYPE_QUALIFIER_OBJ_OWNER) {
      struct type argument_object_type = {0};
      struct object *p_argument_object = expression_get_object(
          p_current_argument->expression, &argument_object_type);

      if (p_argument_object) {
        set_object(&argument_object_type, p_argument_object,
        ...
        if (p_argument_object->pointed) {
          if (type_is_void_ptr(&p_current_parameter_type->type)) {
            ...
            /* moving a owner pointer to non void* moves the pointed object
             * too*/
            if (p_argument_object->pointed) {
...
static void flow_visit_expression(struct flow_visit_ctx *ctx,
  ...
  case POSTFIX_FUNCTION_CALL:
    ...
    flow_visit_argument_expression_list(
        ctx, &p_expression->argument_expression_list);
    compare_function_arguments2(ctx->ctx, &p_expression->left->type,
  ...
  case ASSIGNMENT_EXPRESSION:
    ...
    if (type_is_owner(&p_expression->left->type)) {
      ...
      struct object *const p_right_object =
          expression_get_object(p_expression->right, &right_object_type);
      if (p_right_object) {
      ...
      struct object *const p_left_object =
          expression_get_object(p_expression->left, &left_object_type);

      if (p_left_object) {
        if (constant_value_is_valid(&p_expression->right->constant_value)) {
          if (constant_value_to_ull(&p_expression->right->constant_value) ==
              0) {
...
static void flow_visit_expression_statement(
    struct flow_visit_ctx *ctx,
...
static void
flow_visit_block_item_list(struct flow_visit_ctx *ctx,
...
//
static void flow_visit_iteration_statement(
    struct flow_visit_ctx *ctx,
  ...
  if (p_iteration_statement->secondary_block) {
    ...
    check_defer_and_variables(
        ctx, p_defer, p_iteration_statement->secondary_block->last_token);
...
static void flow_visit_jump_statement(struct flow_visit_ctx *ctx,
                                      struct jump_statement *p_jump_statement) {

  ctx->has_jumps = true;

  if (p_jump_statement->first_token->type == TK_KEYWORD_THROW) {
    check_all_defer_until_try(ctx, ctx->tail_block,
    ...
    */
    if (p_jump_statement->expression_opt) {
      ...
      struct object *p_object =
          expression_get_object(p_jump_statement->expression_opt, &type);
      if (p_object) {
        set_object(&p_jump_statement->expression_opt->type, p_object,
                   OBJECT_STATE_MOVED);
        if (p_object->pointed) {
...
static void
flow_visit_labeled_statement(struct flow_visit_ctx *ctx,
...
static void flow_visit_primary_block(struct flow_visit_ctx *ctx,
...
static void flow_visit_unlabeled_statement(
    struct flow_visit_ctx *ctx,
  ...
  if (p_unlabeled_statement->primary_block) {
    ...
    flow_visit_expression_statement(
        ctx, p_unlabeled_statement->expression_statement);
...
static void flow_visit_static_assert_declaration(
    struct flow_visit_ctx *ctx,
...
static void flow_visit_declaration_specifiers(
    struct flow_visit_ctx *ctx,
    struct declaration_specifiers *p_declaration_specifiers,
    struct type *p_type);

static void
flow_visit_direct_declarator(struct flow_visit_ctx *ctx,
...
static void flow_visit_init_declarator_list(
    struct flow_visit_ctx *ctx,
  ...
  while (p_init_declarator) {
    ...
    if (p_init_declarator->initializer) {
      if (p_init_declarator->initializer->assignment_expression) {
        flow_visit_expression(
            ctx, p_init_declarator->initializer->assignment_expression);
      } else {
        assert(p_init_declarator->initializer->braced_initializer != NULL);
        if (p_init_declarator->initializer->braced_initializer) {
          flow_visit_bracket_initializer_list(
              ctx, p_init_declarator->initializer->braced_initializer);
        }
      }
    }

    if (p_init_declarator->p_declarator->type.category !=
        TYPE_CATEGORY_FUNCTION) {
      // int members =
      // type_get_num_members(&p_init_declarator->p_declarator->type);

      // assert(p_init_declarator->p_declarator->object.state == NULL);
      // p_init_declarator->p_declarator->object.state = move calloc(members,
      ...
      if (p_init_declarator->initializer &&
        ...
        if (p_init_declarator->initializer->assignment_expression->type
          ...
          */
          struct type right_object_type = {0};
          struct object *p_right_object = expression_get_object(
              p_init_declarator->initializer->assignment_expression,
              &right_object_type);
        ...
        set_object(&p_init_declarator->p_declarator->type,
                   &p_init_declarator->p_declarator->object, OBJECT_STATE_ZERO);
      } else {
        set_object(&p_init_declarator->p_declarator->type,
                   &p_init_declarator->p_declarator->object,
                   OBJECT_STATE_UNINITIALIZED);
...
static void
flow_visit_member_declarator(struct flow_visit_ctx *ctx,
                             struct member_declarator *p_member_declarator) {
  if (p_member_declarator->declarator) {
...
static void flow_visit_member_declarator_list(
    struct flow_visit_ctx *ctx,
...
static void
flow_visit_member_declaration(struct flow_visit_ctx *ctx,
                              struct member_declaration *p_member_declaration) {
  if (p_member_declaration->member_declarator_list_opt) {
    flow_visit_specifier_qualifier_list(
  ...
  if (p_member_declaration->member_declarator_list_opt) {
    flow_visit_member_declarator_list(
        ctx, p_member_declaration->member_declarator_list_opt);
  }
}

static void flow_visit_member_declaration_list(
    struct flow_visit_ctx *ctx,
...
static void flow_visit_attribute_specifier(
    struct flow_visit_ctx *ctx,
...
static void flow_visit_struct_or_union_specifier(
  ...
  flow_visit_member_declaration_list(
      ctx, &p_struct_or_union_specifier->member_declaration_list);
}

static void flow_visit_enumerator(struct flow_visit_ctx *ctx,
...
static void
flow_visit_enumerator_list(struct flow_visit_ctx *ctx,
...
static void
flow_visit_typeof_specifier(struct flow_visit_ctx *ctx,
...
static void flow_visit_storage_class_specifier(
    struct flow_visit_ctx *ctx,
...
static void flow_visit_declaration_specifier(
    struct flow_visit_ctx *ctx,
  ...
  if (p_declaration_specifier->storage_class_specifier) {
    flow_visit_storage_class_specifier(
        ctx, p_declaration_specifier->storage_class_specifier);
  }

  if (p_declaration_specifier->type_specifier_qualifier) {
    flow_visit_type_specifier_qualifier(
 ...
 */
static bool
flow_is_last_item_return(struct compound_statement *p_compound_statement) {
...
void flow_visit_declaration(struct flow_visit_ctx *ctx,
  ...
  if (p_declaration->static_assert_declaration) {
    flow_visit_static_assert_declaration(
        ctx, p_declaration->static_assert_declaration);
  }
  ...
  if (p_declaration->function_body) {
    ...
    struct type type = get_function_return_type(
        &p_declaration->init_declarator_list.head->p_declarator->type);
...
void flow_visit_function(struct flow_visit_ctx *ctx,
                         struct declaration *p_declaration) {
