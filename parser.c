...
const char *object_state_to_string(enum object_state e) {
  switch (e) {
  case OBJECT_STATE_STRUCT:
    return "";
  case OBJECT_STATE_UNINITIALIZED:
    return "UNINITIALIZED";
  case OBJECT_STATE_ZERO:
    return "ZERO";
  case OBJECT_STATE_UNKNOWN:
    return "UNKNOWN";
  case OBJECT_STATE_NOT_ZERO:
    return "NOT_ZERO";
  case OBJECT_STATE_MOVED:
    return "MOVED";
  case OBJECT_STATE_NULL_OR_UNINITIALIZED:
    return "NULL_OR_UNINITIALIZED";
  }
  return "";
}
...
void naming_convention_struct_member(struct parser_ctx *ctx,
                                     struct token *token, struct type *type);
void naming_convention_parameter(struct parser_ctx *ctx, struct token *token,
                                 struct type *type);
void naming_convention_global_var(struct parser_ctx *ctx, struct token *token,
                                  struct type *type,
                                  enum storage_class_specifier_flags storage);
void naming_convention_local_var(struct parser_ctx *ctx, struct token *token,
                                 struct type *type);

///////////////////////////////////////////////////////////////////////////////

static bool parser_is_warning_enabled(const struct parser_ctx *ctx,
                                      enum warning w) {
  return (ctx->options.enabled_warnings_stack
              [ctx->options.enabled_warnings_stack_top_index] &
          w) != 0;
}

static void check_open_brace_style(struct parser_ctx *ctx,
                                   struct token *token) {


  if (token->level == 0 && !(token->flags & TK_FLAG_MACRO_EXPANDED) &&
      token->type == '{' && parser_is_warning_enabled(ctx, W_STYLE)) {
    if (ctx->options.style == STYLE_CAKE) {
      if (token->prev->type == TK_BLANKS &&
          token->prev->prev->type == TK_NEWLINE) {
      } else {
        compiler_set_info_with_token(W_STYLE, ctx, token,
                                     "not following correct brace style {");
      }
    }
  }
}

static void check_close_brace_style(struct parser_ctx *ctx,
                                    struct token *token) {
  ...
  if (token->level == 0 && !(token->flags & TK_FLAG_MACRO_EXPANDED) &&
    ...
    if (ctx->options.style == STYLE_CAKE) {
      if (token->prev->type == TK_BLANKS &&
        ...
        compiler_set_info_with_token(
            W_STYLE, ctx, token, "not following correct close brace style }");
      }
    }
  }
}

static void check_func_open_brace_style(struct parser_ctx *ctx,
                                        struct token *token) {
...
void compiler_set_error_with_token(enum error error, struct parser_ctx *ctx,
                                   const struct token *p_token, const char *fmt,
                                   ...) {
  ...
  if (ctx->p_current_function_opt) {
    func_name = ctx->p_current_function_opt->init_declarator_list.head
  ...
  if (ctx->sarif_file) {
    ...
    if (ctx->p_report->error_count + ctx->p_report->warnings_count +
...
_Bool compiler_set_warning_with_token(enum warning w, struct parser_ctx *ctx,
...
void compiler_set_info_with_token(enum warning w, struct parser_ctx *ctx,
                                  const struct token *p_token, const char *fmt,
                                  ...) {
  if (w != W_NONE) {
    if (p_token->level != 0) {
...
struct map_entry *find_variables(struct parser_ctx *ctx, const char *lexeme,
                                 struct scope **ppscope_opt) {
  if (ppscope_opt != NULL)
    *ppscope_opt = NULL;
...
struct enum_specifier *find_enum_specifier(struct parser_ctx *ctx,
                                           const char *lexeme) {
  struct enum_specifier *best = NULL;
  struct scope *scope = ctx->scopes.tail;
  while (scope) {
    struct map_entry *p_entry = hashmap_find(&scope->tags, lexeme);
    if (p_entry && p_entry->type == TAG_TYPE_ENUN_SPECIFIER) {
      best = p_entry->p;
...
find_struct_or_union_specifier(struct parser_ctx *ctx, const char *lexeme) {
  struct struct_or_union_specifier *p = NULL;
  struct scope *scope = ctx->scopes.tail;
  while (scope) {
    struct map_entry *p_entry = hashmap_find(&scope->tags, lexeme);
    if (p_entry && p_entry->type == TAG_TYPE_STRUCT_OR_UNION_SPECIFIER) {
      p = p_entry->p;
      break;
    }
    scope = scope->previous;
...
struct declarator *find_declarator(struct parser_ctx *ctx, const char *lexeme,
                                   struct scope **ppscope_opt) {
...
struct enumerator *find_enumerator(struct parser_ctx *ctx, const char *lexeme,
                                   struct scope **ppscope_opt) {
  struct map_entry *p_entry = find_variables(ctx, lexeme, ppscope_opt);
...
bool first_of_typedef_name(struct parser_ctx *ctx, struct token *p_token) {
  ...
  if (pdeclarator && pdeclarator->declaration_specifiers &&
...
bool first_of_type_specifier_token(struct parser_ctx *ctx,
                                   struct token *p_token) {
  if (p_token == NULL)
    return false;


  return p_token->type == TK_KEYWORD_VOID || p_token->type == TK_KEYWORD_CHAR ||
         p_token->type == TK_KEYWORD_SHORT || p_token->type == TK_KEYWORD_INT ||
         ...
         p_token->type == TK_KEYWORD_TYPEOF_UNQUAL ||
...
bool first_of_iteration_statement(struct parser_ctx *ctx) {
  ...
  return ctx->current->type == TK_KEYWORD_REPEAT || /*extension*/
...
enum token_type is_keyword(const char *text) {
  ...
  case 'r':
    ...
    else if (strcmp("repeat", text) == 0)
      result = TK_KEYWORD_REPEAT;
    break;
  case 's':
    if (strcmp("short", text) == 0)
      ...
      result =
          TK_KEYWORD__STATIC_ASSERT; /*C23 alternate spelling _Static_assert*/
  ...
  case 't':
    if (strcmp("typedef", text) == 0)
      ...
      result =
          TK_KEYWORD__THREAD_LOCAL; /*C23 alternate spelling _Thread_local*/
  ...
  case '_':
    ...
    else if (strcmp("_is_same", text) == 0)
      result = TK_KEYWORD_IS_SAME;
    ...
    else if (strcmp("_Move", text) == 0)
      result = TK_KEYWORD__MOVE; /*extension*/
    else if (strcmp("_Implicit", text) == 0)
      result = TK_KEYWORD__IMPLICIT; /*extension*/
...
void hexadecimal_digit_sequence(struct stream *stream) {
  ...
  */
  ...
  stream_match(stream);
  while (stream->current[0] == '\'' || is_hexadecimal_digit(stream)) {
    if (stream->current[0] == '\'') {
      stream_match(stream);
...
bool first_of_unsigned_suffix(struct stream *stream) {
  /*
   unsigned-suffix: one of
     u U
   */
  return (stream->current[0] == 'u' || stream->current[0] == 'U');
}
...
void integer_suffix_opt(struct stream *stream,
                        enum type_specifier_flags *flags_opt) {
  /*
   integer-suffix:
...
enum token_type parse_number_core(struct stream *stream,
                                  enum type_specifier_flags *flags_opt) {
  if (flags_opt) {
  ...
  if (stream->current[0] == '.') {
    ...
    if (flags_opt) {
      *flags_opt = f;
             ...
             (stream->current[1] == 'x' || stream->current[1] == 'X')) {
    ...
    if (stream->current[0] == 'p' || stream->current[0] == 'P') {
      type = TK_COMPILER_HEXADECIMAL_FLOATING_CONSTANT;
  ...
  } else if (is_nonzero_digit(stream))
  {
...
enum token_type parse_number(const char *lexeme,
                             enum type_specifier_flags *flags_opt) {
  struct stream stream = {
      .source = lexeme, .current = lexeme, .line = 1, .col = 1};
    ...
    if (ctx->current && strcmp(ctx->current->lexeme, "diagnostic") == 0) {
      ...
      if (ctx->current && strcmp(ctx->current->lexeme, "push") == 0) {
        ...
        if (ctx->options.enabled_warnings_stack_top_index <
            sizeof(ctx->options.enabled_warnings_stack) /
                sizeof(ctx->options.enabled_warnings_stack[0])) {
          ctx->options.enabled_warnings_stack_top_index++;
          ctx->options.enabled_warnings_stack
              [ctx->options.enabled_warnings_stack_top_index] =
              ctx->options.enabled_warnings_stack
                  [ctx->options.enabled_warnings_stack_top_index - 1];
        }
        ctx->current = ctx->current->next;
        pragma_skip_blanks(ctx);
        ...
        if (ctx->current && ctx->current->type == TK_STRING_LITERAL) {
          ...
          ctx->options.enabled_warnings_stack
              [ctx->options.enabled_warnings_stack_top_index] |= w;
        }
...
int parser_match_tk(struct parser_ctx *ctx, enum token_type type) {
  ...
  if (ctx->current != NULL) {
    if (ctx->current->type != type) {
      compiler_set_error_with_token(C_UNEXPECTED_TOKEN, ctx, ctx->current,
                                    "expected %s", get_token_name(type));
      error = 1;
    }
    ...
    compiler_set_error_with_token(C_UNEXPECTED_TOKEN, ctx, ctx->input_list.tail,
...
void print_declaration_specifiers(
    struct osstream *ss,
    struct declaration_specifiers *p_declaration_specifiers) {
  bool first = true;
  print_type_qualifier_flags(ss, &first,
                             p_declaration_specifiers->type_qualifier_flags);

  if (p_declaration_specifiers->enum_specifier) {

    if (p_declaration_specifiers->enum_specifier->tag_token)
      ss_fprintf(ss, "enum %s",
                 p_declaration_specifiers->enum_specifier->tag_token->lexeme);
    else
      assert(false);
  } else if (p_declaration_specifiers->struct_or_union_specifier) {
    ss_fprintf(ss, "struct %s",
               p_declaration_specifiers->struct_or_union_specifier->tag_name);
  } else if (p_declaration_specifiers->typedef_declarator) {
    print_item(ss, &first,
               p_declaration_specifiers->typedef_declarator->name->lexeme);
  } else {
    print_type_specifier_flags(ss, &first,
                               p_declaration_specifiers->type_specifier_flags);
  }
}

bool type_specifier_is_integer(enum type_specifier_flags flags) {
  if ((flags & TYPE_SPECIFIER_CHAR) || (flags & TYPE_SPECIFIER_SHORT) ||
      (flags & TYPE_SPECIFIER_INT) || (flags & TYPE_SPECIFIER_LONG) ||
      (flags & TYPE_SPECIFIER_INT) || (flags & TYPE_SPECIFIER_INT8) ||
      (flags & TYPE_SPECIFIER_INT16) || (flags & TYPE_SPECIFIER_INT32) ||
      (flags & TYPE_SPECIFIER_INT64) || (flags & TYPE_SPECIFIER_LONG_LONG)) {
    return true;
  }
  return false;
}
...
int add_specifier(struct parser_ctx *ctx, enum type_specifier_flags *flags,
                  enum type_specifier_flags new_flag) {
  /*
   * Verifica as combinaçòes possíveis
   */

  if (new_flag & TYPE_SPECIFIER_LONG)
  {
    if ((*flags) & TYPE_SPECIFIER_LONG_LONG)
    {
      compiler_set_error_with_token(
          C_CANNOT_COMBINE_WITH_PREVIOUS_LONG_LONG, ctx, ctx->current,
          "cannot combine with previous 'long long' declaration specifier");
      return 1;
    } else if ((*flags) & TYPE_SPECIFIER_LONG)
    {
...
struct declaration_specifiers *owner declaration_specifiers(
  ...
  */
  ...
  try {
    ...
    while (first_of_declaration_specifier(ctx)) {
      if (ctx->current->flags & TK_FLAG_IDENTIFIER_IS_TYPEDEF) {
        if (p_declaration_specifiers->type_specifier_flags !=
            TYPE_SPECIFIER_NONE) {
      ...
      struct declaration_specifier *owner p_declaration_specifier =
          declaration_specifier(ctx);
        ...
        if (p_declaration_specifier->type_specifier_qualifier) {
          if (p_declaration_specifier->type_specifier_qualifier
            ...
            if (add_specifier(ctx,
                              &p_declaration_specifiers->type_specifier_flags,
                              p_declaration_specifier->type_specifier_qualifier
                                  ->type_specifier->flags) != 0) {
              throw;
            }

            if (p_declaration_specifier->type_specifier_qualifier
                    ->type_specifier->struct_or_union_specifier) {
              p_declaration_specifiers->struct_or_union_specifier =
                  p_declaration_specifier->type_specifier_qualifier
                      ->type_specifier->struct_or_union_specifier;
            } else if (p_declaration_specifier->type_specifier_qualifier
                           ->type_specifier->enum_specifier) {
              p_declaration_specifiers->enum_specifier =
                  p_declaration_specifier->type_specifier_qualifier
                      ->type_specifier->enum_specifier;
            } else if (p_declaration_specifier->type_specifier_qualifier
                           ->type_specifier->typeof_specifier) {
              p_declaration_specifiers->typeof_specifier =
                  p_declaration_specifier->type_specifier_qualifier
                      ->type_specifier->typeof_specifier;
            } else if (p_declaration_specifier->type_specifier_qualifier
                           ->type_specifier->token &&
                       p_declaration_specifier->type_specifier_qualifier
                               ->type_specifier->token->type == TK_IDENTIFIER) {
              p_declaration_specifiers->typedef_declarator = find_declarator(
                  ctx,
                  p_declaration_specifier->type_specifier_qualifier
                      ->type_specifier->token->lexeme,
                  NULL);
  ...
  if (p_declaration_specifiers->storage_class_specifier_flags &
...
struct declaration *owner declaration_core(
    struct parser_ctx *ctx,
    struct attribute_specifier_sequence *owner
        p_attribute_specifier_sequence_opt /*SINK*/,
    bool can_be_function_definition, bool *is_function_definition,
...
*/
  ...
  if (first_of_static_assert_declaration(ctx)) {
    p_declaration->static_assert_declaration =
        move static_assert_declaration(ctx);
  } else {

    if (first_of_declaration_specifier(ctx)) {
      p_declaration->declaration_specifiers = move declaration_specifiers(
          ctx, default_storage_class_specifier_flags);
      ...
      if (ctx->current->type != ';') {
        p_declaration->init_declarator_list = move init_declarator_list(
            ctx, p_declaration->declaration_specifiers,
            p_declaration->p_attribute_specifier_sequence_opt);
      }
      ...
      if (ctx->current->type == TK_IDENTIFIER) {
        compiler_set_error_with_token(C_INVALID_TYPE, ctx, ctx->current,
                                      "invalid type '%s'",
                                      ctx->current->lexeme);
      } else {
        compiler_set_error_with_token(C_EXPECTED_DECLARATION, ctx, ctx->current,
                                      "expected declaration not '%s'",
                                      ctx->current->lexeme);
      }
      parser_match(ctx);
    }
...
struct declaration *owner
function_definition_or_declaration(struct parser_ctx *ctx) {
  /*
   function-definition:
  ...
  */
  ...
  /*
    ...
    opt ; attribute-specifier-sequence declaration-specifiers
    init-declarator-list ; static_assert-declaration attribute-declaration
  */
  ...
  struct declaration *owner p_declaration =
      declaration_core(ctx, move p_attribute_specifier_sequence_opt, true,
                       &is_function_definition, STORAGE_SPECIFIER_EXTERN);
  if (is_function_definition) {

    ctx->p_current_function_opt = p_declaration;


    assert(p_declaration->init_declarator_list.head->p_declarator
    ...
    */
    ...
    for (;;) {
      if (inner->direct_declarator &&
          ...
          inner->direct_declarator->function_declarator->direct_declarator
              ->declarator) {
        inner = inner->direct_declarator->function_declarator->direct_declarator
                    ->declarator;
      } else
        break;
    }
    ...
    struct scope *parameters_scope =
        &inner->direct_declarator->function_declarator->parameters_scope;
    ...
    p_declaration->init_declarator_list.head->p_declarator->function_body =
        p_declaration->function_body;
    ...
    if (p_declaration->init_declarator_list.head->p_declarator
            ->direct_declarator->function_declarator &&
        p_declaration->init_declarator_list.head->p_declarator
            ->direct_declarator->function_declarator->parameter_type_list_opt &&
        p_declaration->init_declarator_list.head->p_declarator
            ->direct_declarator->function_declarator->parameter_type_list_opt
            ->parameter_list) {
      parameter = p_declaration->init_declarator_list.head->p_declarator
                      ->direct_declarator->function_declarator
                      ->parameter_type_list_opt->parameter_list->head;
    }
    ...
    while (parameter) {
      if (!type_is_maybe_unused(&parameter->declarator->type) &&
        ...
        if (parameter->declarator->name &&
          ...
          compiler_set_warning_with_token(W_UNUSED_PARAMETER, ctx,
...
struct declaration *owner
declaration(struct parser_ctx *ctx,
            struct attribute_specifier_sequence *owner
                p_attribute_specifier_sequence_opt,
            enum storage_class_specifier_flags storage_specifier_flags) {
  bool is_function_definition;
  return declaration_core(ctx, move p_attribute_specifier_sequence_opt, false,
                          &is_function_definition, storage_specifier_flags);
}
...
struct init_declarator *owner init_declarator(
  ...
  */
  struct init_declarator *owner p_init_declarator =
      calloc(1, sizeof(struct init_declarator));
  try {

    struct token *tkname = 0;
    p_init_declarator->p_declarator =
        move declarator(ctx, NULL, p_declaration_specifiers, false, &tkname);
    ...
    if (tkname == NULL) {
      compiler_set_error_with_token(C_UNEXPECTED, ctx, ctx->current,
                                    "empty declarator name?? unexpected");
    ...
    if (p_init_declarator->p_declarator->declaration_specifiers
      ...
      */
      ...
      p_init_declarator->p_declarator->type =
          move make_type_using_declarator(ctx, p_init_declarator->p_declarator);
    }
      ...
      if (tkname) {
        if (ctx->scopes.tail->scope_level == 0) {
          ...
          if (type_is_function(&p_init_declarator->p_declarator->type)) {
            naming_convention_global_var(
                ctx, tkname, &p_init_declarator->p_declarator->type,
                p_init_declarator->p_declarator->declaration_specifiers
                    ->storage_class_specifier_flags);
          } else {
            naming_convention_global_var(
      ...
      if (previous) {
        if (out->scope_level == ctx->scopes.tail->scope_level) {
          if (out->scope_level == 0) {
            /*file scope*/
            if (!type_is_same(&previous->type,
                              &p_init_declarator->p_declarator->type, true)) {
            ...
            compiler_set_error_with_token(C_REDECLARATION, ctx, ctx->current,
                                          "redeclaration");
            compiler_set_info_with_token(W_NONE, ctx, previous->name,
                                         "previous declaration");
          }
        } else {
          hashmap_set(&ctx->scopes.tail->variables, name, p_init_declarator,
                      TAG_TYPE_INIT_DECLARATOR);

          /*global scope no warning...*/
          if (out->scope_level != 0) {
            /*but redeclaration at function scope we show warning*/
            if (compiler_set_warning_with_token(
                    W_DECLARATOR_HIDE, ctx,
                    p_init_declarator->p_declarator->first_token,
                    "declaration of '%s' hides previous declaration", name)) {
              compiler_set_info_with_token(W_NONE, ctx, previous->first_token,
    ...
    if (ctx->current && ctx->current->type == '=') {
      ...
      if (p_init_declarator->initializer->braced_initializer) {
        if (type_is_array(&p_init_declarator->p_declarator->type)) {
          const int sz =
              type_get_array_size(&p_init_declarator->p_declarator->type);
          if (sz == 0) {
            /*int a[] = {1, 2, 3}*/
            ...
            type_set_array_size(&p_init_declarator->p_declarator->type,
      ...
      if (p_init_declarator->p_declarator->declaration_specifiers
        ...
        if (p_init_declarator->initializer &&
          ...
          if (p_init_declarator->initializer->assignment_expression
                  ->expression_type == UNARY_EXPRESSION_ADDRESSOF) {
            t = move type_dup(
                &p_init_declarator->initializer->assignment_expression->type);
          } else {
            struct type t2 = type_lvalue_conversion(
                &p_init_declarator->initializer->assignment_expression->type);
            type_swap(&t2, &t);
            type_destroy(&t2);
          }
          ...
          type_remove_names(&t);
          t.name_opt =
              move strdup(p_init_declarator->p_declarator->name->lexeme);

          type_set_qualifiers_using_declarator(&t,
                                               p_init_declarator->p_declarator);
  ...
  if (p_init_declarator->initializer &&
      p_init_declarator->initializer->assignment_expression) {
    if (p_init_declarator->initializer->assignment_expression->type
            .type_qualifier_flags &
        TYPE_QUALIFIER_OWNER) {
      if (p_init_declarator->initializer->assignment_expression
              ->expression_type == POSTFIX_FUNCTION_CALL) {

        if (!(p_init_declarator->p_declarator->type.type_qualifier_flags &
              TYPE_QUALIFIER_OWNER)) {
          compiler_set_error_with_token(
              C_MISSING_OWNER, ctx,
              p_init_declarator->p_declarator->first_token,
  ...
  if (p_init_declarator->p_declarator) {
    if (type_is_array(&p_init_declarator->p_declarator->type))
      if (p_init_declarator->p_declarator->type.type_qualifier_flags != 0 ||
        ...
        compiler_set_error_with_token(
            C_STATIC_OR_TYPE_QUALIFIERS_NOT_ALLOWED_IN_NON_PARAMETER, ctx,
            p_init_declarator->p_declarator->first_token,
            "static or type qualifiers are not allowed in non-parameter array "
            "declarator");
      }

    if (!type_is_pointer(&p_init_declarator->p_declarator->type) &&
        p_init_declarator->p_declarator->type.type_qualifier_flags &
            TYPE_QUALIFIER_OBJ_OWNER) {
      compiler_set_error_with_token(
          C_OBJ_OWNER_CAN_BE_USED_ONLY_IN_POINTER, ctx,
          p_init_declarator->p_declarator->first_token,
...
struct init_declarator_list init_declarator_list(
  ...
  */
  ...
  try {
    p_init_declarator = move init_declarator(
        ctx, p_declaration_specifiers, p_attribute_specifier_sequence_opt);
...
struct storage_class_specifier *owner
storage_class_specifier(struct parser_ctx *ctx) {
  if (ctx->current == NULL)
    return NULL;

  struct storage_class_specifier *owner new_storage_class_specifier =
...
struct typeof_specifier_argument *owner
typeof_specifier_argument(struct parser_ctx *ctx) {
  struct typeof_specifier_argument *owner new_typeof_specifier_argument =
      calloc(1, sizeof(struct typeof_specifier_argument));
...
struct typeof_specifier *owner typeof_specifier(struct parser_ctx *ctx) {
  ...
  try {
    ...
    p_typeof_specifier->typeof_specifier_argument =
        move typeof_specifier_argument(ctx);
    if (p_typeof_specifier->typeof_specifier_argument == NULL)
      throw;

    if (p_typeof_specifier->typeof_specifier_argument->expression) {
      p_typeof_specifier->type = move type_dup(
          &p_typeof_specifier->typeof_specifier_argument->expression->type);
    } else if (p_typeof_specifier->typeof_specifier_argument->type_name) {
      p_typeof_specifier->type =
          move type_dup(&p_typeof_specifier->typeof_specifier_argument
                             ->type_name->declarator->type);
    }

    if (p_typeof_specifier->type.storage_class_specifier_flags &
        STORAGE_SPECIFIER_PARAMETER) {
      compiler_set_warning_with_token(W_TYPEOF_ARRAY_PARAMETER, ctx,
                                      ctx->current,
                                      "typeof used in array arguments");
...
struct type_specifier *owner type_specifier(struct parser_ctx *ctx) {
  ...
  */
  ...
  struct type_specifier *owner p_type_specifier =
      calloc(1, sizeof *p_type_specifier);
  ...
  if (first_of_typeof_specifier(ctx)) {
    ...
    p_type_specifier->struct_or_union_specifier =
        move struct_or_union_specifier(ctx);
...
struct struct_or_union_specifier *get_complete_struct_or_union_specifier(
  ...
  if (p_struct_or_union_specifier->member_declaration_list.head) {
    ...
    return p_struct_or_union_specifier;
                 ...
                 ->complete_struct_or_union_specifier_indirection &&
             p_struct_or_union_specifier
                 ->complete_struct_or_union_specifier_indirection
                 ->member_declaration_list.head) {
    /*p_struct_or_union_specifier is the first seem tag tag points directly to
     * complete*/
    return p_struct_or_union_specifier
        ->complete_struct_or_union_specifier_indirection;
  } else if (p_struct_or_union_specifier
...
bool struct_or_union_specifier_is_complete(
    struct struct_or_union_specifier *p_struct_or_union_specifier) {
  return get_complete_struct_or_union_specifier(p_struct_or_union_specifier) !=
         NULL;
}

struct struct_or_union_specifier *owner
struct_or_union_specifier(struct parser_ctx *ctx) {
  struct struct_or_union_specifier *owner p_struct_or_union_specifier =
      calloc(1, sizeof *p_struct_or_union_specifier);
  ...
  if (ctx->current->type == TK_IDENTIFIER) {
    ...
    */
    ...
    snprintf(p_struct_or_union_specifier->tag_name,
             sizeof p_struct_or_union_specifier->tag_name, "%s",
             ctx->current->lexeme);

    struct map_entry *p_entry =
        hashmap_find(&ctx->scopes.tail->tags, ctx->current->lexeme);
    if (p_entry) {
      ...
      if (p_entry->type == TAG_TYPE_STRUCT_OR_UNION_SPECIFIER) {
        ...
        p_struct_or_union_specifier
            ->complete_struct_or_union_specifier_indirection =
            p_first_tag_in_this_scope;
      } else {
        compiler_set_error_with_token(
            C_TAG_TYPE_DOES_NOT_MATCH_PREVIOUS_DECLARATION, ctx, ctx->current,
            "use of '%s' with tag type that does not match previous "
            "declaration.",
            ctx->current->lexeme);
      }
    } else {
      ...
      struct struct_or_union_specifier *p_first_tag_previous_scopes =
          find_struct_or_union_specifier(ctx, ctx->current->lexeme);
      if (p_first_tag_previous_scopes == NULL) {
        /*tag not found, so it is the first appearence*/
        p_struct_or_union_specifier->scope_level =
            ctx->scopes.tail->scope_level;

        hashmap_set(&ctx->scopes.tail->tags, ctx->current->lexeme,
                    p_struct_or_union_specifier,
    ...
    snprintf(p_struct_or_union_specifier->tag_name,
             ...
             "_anonymous_struct_%d", s_anonymous_struct_count);
    ...
    hashmap_set(&ctx->scopes.tail->tags, p_struct_or_union_specifier->tag_name,
                p_struct_or_union_specifier,
                TAG_TYPE_STRUCT_OR_UNION_SPECIFIER);
  }
  ...
  if (ctx->current->type == '{') {
    ...
    */
    ...
    struct struct_or_union_specifier *first = find_struct_or_union_specifier(
        ctx, p_struct_or_union_specifier->tag_name);
    if (first) {
      /*
         The first tag (will the one at symbol table) will point to the complete
         struct
      */
      first->complete_struct_or_union_specifier_indirection =
          p_struct_or_union_specifier;
    }
    ...
    parser_match(ctx);
    p_struct_or_union_specifier->member_declaration_list =
        move member_declaration_list(ctx, p_struct_or_union_specifier);
    p_struct_or_union_specifier->member_declaration_list.first_token =
        firsttoken;
    p_struct_or_union_specifier->last_token = ctx->current;
    p_struct_or_union_specifier->member_declaration_list.last_token =
        ctx->current;
    parser_match_tk(ctx, '}');
  ...
  if (p_complete) {
    if (p_complete->attribute_specifier_sequence_opt &&
        p_complete->attribute_specifier_sequence_opt->attributes_flags &
            STD_ATTRIBUTE_DEPRECATED) {
      if (p_struct_or_union_specifier->tagtoken) {

        compiler_set_warning_with_token(
            W_DEPRECATED, ctx, p_struct_or_union_specifier->first_token,
            "'%s' is deprecated",
...
struct member_declarator *owner
member_declarator(struct parser_ctx *ctx,
                  struct struct_or_union_specifier *p_struct_or_union_specifier,
  ...
  */
  struct member_declarator *owner p_member_declarator =
      calloc(1, sizeof(struct member_declarator));

  struct token *p_token_name = NULL;

  p_member_declarator->declarator =
      move declarator(ctx, p_specifier_qualifier_list,
                      /*declaration_specifiers*/ NULL, false, &p_token_name);
  p_member_declarator->declarator->name = p_token_name;
  p_member_declarator->declarator->specifier_qualifier_list =
      p_specifier_qualifier_list;

  p_member_declarator->declarator->type =
      move make_type_using_declarator(ctx, p_member_declarator->declarator);

  /*extension*/
  if (p_member_declarator->declarator->type.type_qualifier_flags &
      TYPE_QUALIFIER_OWNER) {
  ...
  if (p_member_declarator->declarator->name)
    naming_convention_struct_member(ctx, p_member_declarator->declarator->name,
                                    &p_member_declarator->declarator->type);
...
struct member_declarator_list *owner member_declarator_list(
    ...
    struct struct_or_union_specifier *p_struct_or_union_specifier,
    struct specifier_qualifier_list *p_specifier_qualifier_list) {
  struct member_declarator_list *owner p_member_declarator_list =
      calloc(1, sizeof(struct member_declarator_list));
  LIST_ADD(p_member_declarator_list,
           member_declarator(ctx, p_struct_or_union_specifier,
                             p_specifier_qualifier_list));
...
struct member_declaration_list member_declaration_list(
  ...
  try {
    p_member_declaration =
        move member_declaration(ctx, p_struct_or_union_specifier);
...
struct member_declaration *owner member_declaration(
    struct parser_ctx *ctx,
    struct struct_or_union_specifier *p_struct_or_union_specifier) {
  struct member_declaration *owner p_member_declaration =
      calloc(1, sizeof(struct member_declaration));


  if (ctx->current->type == TK_KEYWORD__STATIC_ASSERT) {
    p_member_declaration->static_assert_declaration =
        move static_assert_declaration(ctx);
  } else {
    p_member_declaration->p_attribute_specifier_sequence_opt =
        move attribute_specifier_sequence_opt(ctx);

    p_member_declaration->specifier_qualifier_list =
        move specifier_qualifier_list(ctx);
    if (ctx->current->type != ';') {
      p_member_declaration->member_declarator_list_opt =
          move member_declarator_list(
              ctx, p_struct_or_union_specifier,
              p_member_declaration->specifier_qualifier_list);
    }
    parser_match_tk(ctx, ';');
  }
...
find_member_declarator(struct member_declaration_list *list, const char *name,
                       int *p_member_index) {
  int member_index = 0;
      ...
      */
      ...
      if (p_member_declaration->specifier_qualifier_list
        ...
        struct member_declaration_list *p_member_declaration_list =
            &p_member_declaration->specifier_qualifier_list
                 ->struct_or_union_specifier->member_declaration_list;
        int inner_member_index = 0;
        struct member_declarator *p = find_member_declarator(
            p_member_declaration_list, name, &inner_member_index);
        if (p) {
...
void print_specifier_qualifier_list(
    struct osstream *ss, bool *first,
    struct specifier_qualifier_list *p_specifier_qualifier_list) {

  print_type_qualifier_flags(ss, first,
                             p_specifier_qualifier_list->type_qualifier_flags);
  ...
  if (p_specifier_qualifier_list->enum_specifier) {
    ...
    ss_fprintf(ss, "struct %s",
               p_specifier_qualifier_list->struct_or_union_specifier->tag_name);
  } else if (p_specifier_qualifier_list->typedef_declarator) {
    print_item(ss, first,
               p_specifier_qualifier_list->typedef_declarator->name->lexeme);
  } else {
    print_type_specifier_flags(
        ss, first, p_specifier_qualifier_list->type_specifier_flags);
  }
}

struct specifier_qualifier_list *owner
specifier_qualifier_list(struct parser_ctx *ctx) {
  struct specifier_qualifier_list *owner p_specifier_qualifier_list =
  ...
  */
  try {
    ...
    while (ctx->current != NULL &&
           (first_of_type_specifier(ctx) || first_of_type_qualifier(ctx))) {

      if (ctx->current->flags & TK_FLAG_IDENTIFIER_IS_TYPEDEF) {
        if (p_specifier_qualifier_list->type_specifier_flags !=
            TYPE_SPECIFIER_NONE) {
      ...
      struct type_specifier_qualifier *owner p_type_specifier_qualifier =
          type_specifier_qualifier(ctx);

      if (p_type_specifier_qualifier->type_specifier) {
        if (add_specifier(
                ctx, &p_specifier_qualifier_list->type_specifier_flags,
        ...
        if (p_type_specifier_qualifier->type_specifier
                ->struct_or_union_specifier) {
          p_specifier_qualifier_list->struct_or_union_specifier =
              p_type_specifier_qualifier->type_specifier
                  ->struct_or_union_specifier;
        } else if (p_type_specifier_qualifier->type_specifier->enum_specifier) {
          p_specifier_qualifier_list->enum_specifier =
              p_type_specifier_qualifier->type_specifier->enum_specifier;
        } else if (p_type_specifier_qualifier->type_specifier
                       ->typeof_specifier) {
          p_specifier_qualifier_list->typeof_specifier =
              p_type_specifier_qualifier->type_specifier->typeof_specifier;
        } else if (p_type_specifier_qualifier->type_specifier->token->type ==
                   TK_IDENTIFIER) {
          p_specifier_qualifier_list->typedef_declarator = find_declarator(
              ctx, p_type_specifier_qualifier->type_specifier->token->lexeme,
              NULL);
        }

      } else if (p_type_specifier_qualifier->type_qualifier) {
        p_specifier_qualifier_list->type_qualifier_flags |=
            p_type_specifier_qualifier->type_qualifier->flags;
      }
...
struct type_specifier_qualifier *owner
type_specifier_qualifier(struct parser_ctx *ctx) {
  struct type_specifier_qualifier *owner type_specifier_qualifier =
      calloc(1, sizeof *type_specifier_qualifier);
  ...
  if (first_of_type_specifier(ctx)) {
    ...
    type_specifier_qualifier->alignment_specifier =
        move alignment_specifier(ctx);
  } else {
    assert(false);
  }
...
struct enum_specifier *owner enum_specifier(struct parser_ctx *ctx) {
  ...
  */
  ...
  /*
      ...
      "enum" attribute-specifier-sequence opt identifier opt enum-type-specifier
     opt { enumerator-list }

      "enum" attribute-specifier-sequence opt identifier opt enum-type-specifier
     opt { enumerator-list , } enum identifier enum-type-specifier opt
  */
  ...
  try {
    ...
    if (ctx->current->type == '{') {
      ...
      parser_match_tk(ctx, '{');
      p_enum_specifier->enumerator_list =
          move enumerator_list(ctx, p_enum_specifier);
      if (ctx->current->type == ',') {
        parser_match(ctx);
      }
      parser_match_tk(ctx, '}');
      ...
      if (!has_identifier) {
        compiler_set_error_with_token(C_MISSING_ENUM_TAG_NAME, ctx,
                                      ctx->current, "missing enum tag name");
        throw;
      }
    }
    ...
    if (p_enum_specifier->tag_token && p_enum_specifier->tag_token->lexeme) {
      p_entry = hashmap_find(&ctx->scopes.tail->tags,
      ...
      */
      if (p_entry->type == TAG_TYPE_ENUN_SPECIFIER) {
        ...
        if (p_previous_tag_in_this_scope->enumerator_list.head != NULL &&
          ...
          compiler_set_error_with_token(C_MULTIPLE_DEFINITION_ENUM, ctx,
                                        p_enum_specifier->tag_token,
                                        "multiple definition of 'enum %s'",
                                        p_enum_specifier->tag_token->lexeme);
        } else if (p_previous_tag_in_this_scope->enumerator_list.head != NULL) {
          p_enum_specifier->complete_enum_specifier =
              p_previous_tag_in_this_scope;
        } else if (p_enum_specifier->enumerator_list.head != NULL) {
          p_previous_tag_in_this_scope->complete_enum_specifier =
              p_enum_specifier;
        }
      } else {
        compiler_set_error_with_token(
      ...
      if (p_other == NULL) {
        ...
        if (p_enum_specifier->tag_token) {
          hashmap_set(&ctx->scopes.tail->tags,
                      p_enum_specifier->tag_token->lexeme, p_enum_specifier,
...
enumerator_list(struct parser_ctx *ctx,
...
struct enumerator *owner enumerator(struct parser_ctx *ctx,
  ...
  hashmap_set(&ctx->scopes.tail->variables, p_enumerator->token->lexeme,
              p_enumerator, TAG_TYPE_ENUMERATOR);

  if (ctx->current->type == '=') {
    parser_match(ctx);
    p_enumerator->constant_expression_opt = move constant_expression(ctx);
    p_enumerator->value = constant_value_to_ll(
        &p_enumerator->constant_expression_opt->constant_value);
  }
...
struct alignment_specifier *owner alignment_specifier(struct parser_ctx *ctx) {
  struct alignment_specifier *owner alignment_specifier =
      calloc(1, sizeof *alignment_specifier);
...
struct atomic_type_specifier *owner
atomic_type_specifier(struct parser_ctx *ctx) {
  //'_Atomic' '(' type_name ')'
...
struct type_qualifier *owner type_qualifier(struct parser_ctx *ctx) {
  struct type_qualifier *owner p_type_qualifier =
      calloc(1, sizeof *p_type_qualifier);
...
struct function_specifier *owner function_specifier(struct parser_ctx *ctx) {
  if (ctx->current->type == TK_KEYWORD__NORETURN) {
    compiler_set_info_with_token(W_STYLE, ctx, ctx->current,
                                 "_Noreturn is deprecated use attributes");
  }
...
struct declarator *owner
declarator(struct parser_ctx *ctx,
           struct specifier_qualifier_list *p_specifier_qualifier_list,
           struct declaration_specifiers *p_declaration_specifiers,
           bool abstract_acceptable, struct token **pp_token_name) {
  /*
    declarator:
  ...
  */
  ...
  p_declarator->direct_declarator = move direct_declarator(
      ctx, p_specifier_qualifier_list, p_declaration_specifiers,
      abstract_acceptable, pp_token_name);
  ...
  if (ctx->current != p_declarator->first_token) {
    ...
    p_declarator->first_token =
        NULL; /*this is the way we can know...first is null*/
  }
...
struct array_declarator *owner
array_declarator(struct direct_declarator *owner p_direct_declarator,
                 struct parser_ctx *ctx);
struct function_declarator *owner
function_declarator(struct direct_declarator *owner p_direct_declarator,
                    struct parser_ctx *ctx);

struct direct_declarator *owner
direct_declarator(struct parser_ctx *ctx,
                  struct specifier_qualifier_list *p_specifier_qualifier_list,
                  struct declaration_specifiers *p_declaration_specifiers,
                  bool abstract_acceptable, struct token **pptoken_name) {
  /*
  direct-declarator:
  ...
  */
  struct direct_declarator *owner p_direct_declarator =
  ...
  try {
    ...
    if (ctx->current->type == TK_IDENTIFIER) {
      ...
      parser_match(ctx);
      p_direct_declarator->p_attribute_specifier_sequence_opt =
          move attribute_specifier_sequence_opt(ctx);
    } else if (ctx->current->type == '(') {
      struct token *ahead = parser_look_ahead(ctx);

      if (!first_of_type_specifier_token(ctx, p_token_ahead) &&
          !first_of_type_qualifier_token(p_token_ahead) && ahead->type != ')' &&
          ahead->type != '...') {
        ...
        p_direct_declarator->declarator = move declarator(
            ctx, p_specifier_qualifier_list, p_declaration_specifiers,
            abstract_acceptable, pptoken_name);
    ...
    while (ctx->current != NULL &&
      ...
      struct direct_declarator *owner p_direct_declarator2 =
          calloc(1, sizeof(struct direct_declarator));

      if (ctx->current->type == '[') {
        p_direct_declarator2->array_declarator =
            move array_declarator(move p_direct_declarator, ctx);
      } else {
        p_direct_declarator2->function_declarator =
            move function_declarator(move p_direct_declarator, ctx);
      }
...
unsigned long long
array_declarator_get_size(struct array_declarator *p_array_declarator) {
  if (p_array_declarator->assignment_expression) {
    if (constant_value_is_valid(
            &p_array_declarator->assignment_expression->constant_value)) {
      return constant_value_to_ull(
          &p_array_declarator->assignment_expression->constant_value);
    }
  }
  return 0;
...
array_declarator(struct direct_declarator *owner p_direct_declarator,
  ...
  try {
    ...
    if (first_of_type_qualifier(ctx)) {
      p_array_declarator->type_qualifier_list_opt =
          move type_qualifier_list(ctx);
    }
    ...
    if (has_static) {
      ...
      p_array_declarator->assignment_expression =
          move assignment_expression(ctx);
...
function_declarator(struct direct_declarator *owner p_direct_declarator,
                    struct parser_ctx *ctx) {
  struct function_declarator *owner p_function_declarator =
      calloc(1, sizeof(struct function_declarator));
  ...
  p_function_declarator->parameters_scope.scope_level =
      ctx->scopes.tail->scope_level + 1;
...
struct parameter_type_list *owner parameter_type_list(struct parser_ctx *ctx) {
  struct parameter_type_list *owner p_parameter_type_list =
  ...
  if (p_parameter_type_list->parameter_list->head ==
    ...
    if (type_is_void(
            &p_parameter_type_list->parameter_list->head->declarator->type)) {
...
struct parameter_declaration *owner
parameter_declaration(struct parser_ctx *ctx) {
  struct parameter_declaration *owner p_parameter_declaration =
  ...
  p_parameter_declaration->declaration_specifiers =
      move declaration_specifiers(ctx, STORAGE_SPECIFIER_PARAMETER);
  ...
  p_parameter_declaration->declarator =
      move declarator(ctx,
                      ...
                      p_parameter_declaration->declaration_specifiers,
                      true /*can be abstract*/, &p_token_name);
  p_parameter_declaration->declarator->name = p_token_name;

  p_parameter_declaration->declarator->declaration_specifiers =
      p_parameter_declaration->declaration_specifiers;

  p_parameter_declaration->declarator->type =
      move make_type_using_declarator(ctx, p_parameter_declaration->declarator);
  ...
  p_parameter_declaration->declarator->type.storage_class_specifier_flags |=
      STORAGE_SPECIFIER_PARAMETER;

  if (p_parameter_declaration->implicit_token) {
    p_parameter_declaration->declarator->type.attributes_flags |=
        CAKE_ATTRIBUTE_IMPLICT;
  }

  if (p_parameter_declaration->declarator->name)
    naming_convention_parameter(ctx, p_parameter_declaration->declarator->name,
                                &p_parameter_declaration->declarator->type);
  ...
  if (p_parameter_declaration->declarator->name) {
    ...
    hashmap_set(&ctx->scopes.tail->variables,
                ...
                p_parameter_declaration->declarator, TAG_TYPE_ONLY_DECLARATOR);
...
struct specifier_qualifier_list *owner
copy(struct declaration_specifiers *p_declaration_specifiers) {
  struct specifier_qualifier_list *owner p_specifier_qualifier_list =
      calloc(1, sizeof(struct specifier_qualifier_list));
  ...
  while (p_declaration_specifier) {
    if (p_declaration_specifier->type_specifier_qualifier) {
      struct type_specifier_qualifier *owner p_specifier_qualifier =
          calloc(1, sizeof(struct type_specifier_qualifier));

      if (p_declaration_specifier->type_specifier_qualifier->type_qualifier) {
        struct type_qualifier *owner p_type_qualifier =
            calloc(1, sizeof(struct type_qualifier));

        p_type_qualifier->flags =
            p_declaration_specifier->type_specifier_qualifier->type_qualifier
                ->flags;

        p_type_qualifier->token =
            p_declaration_specifier->type_specifier_qualifier->type_qualifier
                ->token;
        p_specifier_qualifier->type_qualifier = move p_type_qualifier;
                     ...
                     ->type_specifier) {
        struct type_specifier *owner p_type_specifier =
            calloc(1, sizeof(struct type_specifier));

        p_type_specifier->flags =
            p_declaration_specifier->type_specifier_qualifier->type_specifier
                ->flags;


        assert(p_declaration_specifier->type_specifier_qualifier->type_specifier
...
void print_declarator(struct osstream *ss, struct declarator *p_declarator,
                      bool is_abstract);

void print_direct_declarator(struct osstream *ss,
  ...
  if (p_direct_declarator->function_declarator) {
    print_direct_declarator(
        ss, p_direct_declarator->function_declarator->direct_declarator,
        is_abstract);
    ...
    while (p_parameter_declaration) {
      if (p_parameter_declaration !=
          p_direct_declarator->function_declarator->parameter_type_list_opt
              ->parameter_list->head)
        ss_fprintf(ss, ",");

      print_declaration_specifiers(
          ss, p_parameter_declaration->declaration_specifiers);
      ss_fprintf(ss, " ");
...
declarator_get_type_specifier_flags(const struct declarator *p) {
  ...
  if (p_declarator->pointer) {
    ...
    while (p) {
      if (p->type_qualifier_list_opt) {
        print_type_qualifier_flags(ss, &first,
                                   p->type_qualifier_list_opt->flags);
      }
      ss_fprintf(ss, "*");
      p = p->pointer;
    }
...
struct type_name *owner type_name(struct parser_ctx *ctx) {
  ...
  p_type_name->declarator = move declarator(
      ctx,
      p_type_name->specifier_qualifier_list, //??
      /*declaration_specifiers*/ NULL, true /*DEVE SER TODO*/, NULL);
  p_type_name->declarator->specifier_qualifier_list =
      p_type_name->specifier_qualifier_list;
  p_type_name->declarator->type =
      move make_type_using_declarator(ctx, p_type_name->declarator);
...
struct braced_initializer *owner braced_initializer(struct parser_ctx *ctx) {
  ...
  */
  ...
  struct braced_initializer *owner p_bracket_initializer_list =
      calloc(1, sizeof(struct braced_initializer));
...
struct initializer *owner initializer(struct parser_ctx *ctx) {
  ...
  */
  ...
  struct initializer *owner p_initializer =
...
struct initializer_list *owner initializer_list(struct parser_ctx *ctx) {
  ...
  */
  ...
  struct initializer_list *owner p_initializer_list =
...
struct designation *owner designation(struct parser_ctx *ctx) {
  ...
  struct designation *owner p_designation =
      calloc(1, sizeof(struct designation));
...
struct static_assert_declaration *owner
static_assert_declaration(struct parser_ctx *ctx) {
  ...
  */
  ...
  try {
    p_static_assert_declaration =
        move calloc(1, sizeof(struct static_assert_declaration));
    ...
    p_static_assert_declaration->constant_expression =
        move constant_expression(ctx);
    ...
    if (!constant_value_to_bool(&p_static_assert_declaration
                                     ->constant_expression->constant_value)) {
      if (p_static_assert_declaration->string_literal_opt) {
        ...
        compiler_set_error_with_token(C_STATIC_ASSERT_FAILED, ctx, position,
                                      "_Static_assert failed");
      }

      if (p_static_assert_declaration->constant_expression->expression_type ==
          EQUALITY_EXPRESSION_EQUAL) {
        if (p_static_assert_declaration->constant_expression->left
                ->expression_type == UNARY_EXPRESSION_STATIC_DEBUG) {
          compiler_set_info_with_token(
              W_NONE, ctx, position, "%llu != %llu",
              constant_value_to_ull(
                  &p_static_assert_declaration->constant_expression->left
                       ->constant_value),
              constant_value_to_ull(
                  &p_static_assert_declaration->constant_expression->right
                       ->constant_value));
        }
      }
    }
...
struct attribute_specifier_sequence *owner
attribute_specifier_sequence_opt(struct parser_ctx *ctx) {
  ...
  if (first_of_attribute_specifier(ctx)) {
    p_attribute_specifier_sequence =
        move calloc(1, sizeof(struct attribute_specifier_sequence));

    p_attribute_specifier_sequence->first_token = ctx->current;

    while (ctx->current != NULL && first_of_attribute_specifier(ctx)) {
      struct attribute_specifier *owner p_attribute_specifier =
...
struct attribute_specifier_sequence *owner
attribute_specifier_sequence(struct parser_ctx *ctx) {
...
struct attribute_specifier *owner attribute_specifier(struct parser_ctx *ctx) {
  struct attribute_specifier *owner p_attribute_specifier =
      calloc(1, sizeof(struct attribute_specifier));
...
struct attribute_list *owner attribute_list(struct parser_ctx *ctx) {
  struct attribute_list *owner p_attribute_list =
      calloc(1, sizeof(struct attribute_list));
  //

  while (ctx->current != NULL &&
         (first_of_attribute(ctx) || ctx->current->type == ',')) {
...
struct attribute *owner attribute(struct parser_ctx *ctx) {
  ...
  p_attribute->attributes_flags =
      p_attribute->attribute_token->attributes_flags;
  if (ctx->current->type == '(')
  {
    p_attribute->attribute_argument_clause =
        move attribute_argument_clause(ctx);
  }
  return p_attribute;
}

struct attribute_token *owner attribute_token(struct parser_ctx *ctx) {
  struct attribute_token *owner p_attribute_token =
      calloc(1, sizeof(struct attribute_token));
  ...
  if (ctx->current->type == '::') {
    ...
    if (is_cake_attr) {
      ...
      {
        compiler_set_warning_with_token(W_ATTRIBUTES, ctx, attr_token,
                                        "warning '%s' is not an cake attribute",
                                        ctx->current->lexeme);
      }
    }
    parser_match_tk(ctx, TK_IDENTIFIER);
    ...
    /*
     * Each implementation should choose a distinctive name for the attribute
     * prefix in an attribute prefixed token. Implementations should not define
     * attributes without an attribute prefix unless it is a standard attribute
     * as specified in this document.
     */
    if (!is_standard_attribute) {
      compiler_set_warning_with_token(
          W_ATTRIBUTES, ctx, attr_token,
          "warning '%s' is not an standard attribute", attr_token->lexeme);
    }
  }
  return p_attribute_token;
}

struct attribute_argument_clause *owner
attribute_argument_clause(struct parser_ctx *ctx) {
  struct attribute_argument_clause *owner p_attribute_argument_clause =
      calloc(1, sizeof(struct attribute_argument_clause));
  //'(' balanced_token_sequence_opt ')'
  parser_match_tk(ctx, '(');
  p_attribute_argument_clause->p_balanced_token_sequence =
      move balanced_token_sequence_opt(ctx);
  parser_match_tk(ctx, ')');
  return p_attribute_argument_clause;
}

struct balanced_token_sequence *owner
balanced_token_sequence_opt(struct parser_ctx *ctx) {
  struct balanced_token_sequence *owner p_balanced_token_sequence =
      calloc(1, sizeof(struct balanced_token_sequence));
  ...
  if (count2 != 0) {
    compiler_set_error_with_token(C_ATTR_UNBALANCED, ctx, ctx->current,
                                  "expected ']' before ')'");
  }
  if (count3 != 0) {
    compiler_set_error_with_token(C_ATTR_UNBALANCED, ctx, ctx->current,
...
struct primary_block *owner primary_block(struct parser_ctx *ctx) {
  ...
  struct primary_block *owner p_primary_block =
      calloc(1, sizeof(struct primary_block));
...
struct secondary_block *owner secondary_block(struct parser_ctx *ctx) {
  ...
  struct secondary_block *owner p_secondary_block =
      calloc(1, sizeof(struct secondary_block));
...
bool first_of_primary_block(struct parser_ctx *ctx) {
  if (first_of_compound_statement(ctx) || first_of_selection_statement(ctx) ||
...
struct unlabeled_statement *owner unlabeled_statement(struct parser_ctx *ctx) {
  ...
  */
  struct unlabeled_statement *owner p_unlabeled_statement =
      calloc(1, sizeof(struct unlabeled_statement));
  ...
  if (first_of_primary_block(ctx)) {
    ...
    p_unlabeled_statement->expression_statement =
        move expression_statement(ctx);
    if (p_unlabeled_statement->expression_statement) {
      if (p_unlabeled_statement->expression_statement->expression_opt) {
        if (!type_is_void(&p_unlabeled_statement->expression_statement
                               ->expression_opt->type) &&
            type_is_nodiscard(&p_unlabeled_statement->expression_statement
                                   ->expression_opt->type) &&
            p_unlabeled_statement->expression_statement->expression_opt->type
                    .storage_class_specifier_flags &
                STORAGE_SPECIFIER_FUNCTION_RETURN) {
        ...
        if (type_is_owner(&p_unlabeled_statement->expression_statement
                               ->expression_opt->type) &&
            p_unlabeled_statement->expression_statement->expression_opt->type
                    .storage_class_specifier_flags &
                STORAGE_SPECIFIER_FUNCTION_RETURN) {
      ...
      if (p_unlabeled_statement->expression_statement->expression_opt &&
        ...
        if (p_unlabeled_statement != NULL &&
            ...
            p_unlabeled_statement->expression_statement->expression_opt &&
            !type_is_void(&p_unlabeled_statement->expression_statement
                               ->expression_opt->type) &&
            p_unlabeled_statement->expression_statement->expression_opt
                    ->expression_type != ASSIGNMENT_EXPRESSION &&
            p_unlabeled_statement->expression_statement->expression_opt
                    ->expression_type != POSTFIX_FUNCTION_CALL &&
            p_unlabeled_statement->expression_statement->expression_opt
            ...
            p_unlabeled_statement->expression_statement->expression_opt
                    ->expression_type != UNARY_EXPRESSION_INCREMENT &&
            p_unlabeled_statement->expression_statement->expression_opt
                    ->expression_type != UNARY_EXPRESSION_DECREMENT &&
            p_unlabeled_statement->expression_statement->expression_opt
                    ->expression_type != UNARY_DECLARATOR_ATTRIBUTE_EXPR) {
...
struct labeled_statement *owner labeled_statement(struct parser_ctx *ctx) {
  struct labeled_statement *owner p_labeled_statement =
...
struct compound_statement *owner compound_statement(struct parser_ctx *ctx) {
  //'{' block_item_list_opt '}'
  struct compound_statement *owner p_compound_statement =
      ...
      if (p_declarator) {
        ...
        if (!type_is_maybe_unused(&p_declarator->type) &&
          ...
          if (p_declarator->name->token_origin->level == 0) {
            compiler_set_warning_with_token(
                W_UNUSED_VARIABLE, ctx, p_declarator->name,
                "'%s': unreferenced declarator", p_declarator->name->lexeme);
          }
        }
      }
...
struct block_item *owner block_item(struct parser_ctx *ctx) {
  ...
  if (ctx->current->type ==
    ...
    p_block_item->declaration =
        move declaration(ctx, move p_attribute_specifier_sequence_opt,
                         STORAGE_SPECIFIER_AUTOMATIC_STORAGE);

    p_attribute_specifier_sequence_opt = NULL; /*MOVED*/

    struct init_declarator *p =
        p_block_item->declaration->init_declarator_list.head;
    while (p) {
      if (p->p_declarator && p->p_declarator->name) {
        naming_convention_local_var(ctx, p->p_declarator->name,
                                    &p->p_declarator->type);
      }
      p = p->next;
    }
  ...
  /*
                                         declaration-specifiers
     init-declarator-list_opt; attribute-specifier-sequence
     declaration-specifiers init-declarator-list; static_assert-declaration
...
struct try_statement *owner try_statement(struct parser_ctx *ctx) {
  struct try_statement *owner p_try_statement =
      calloc(1, sizeof(struct try_statement));

  p_try_statement->first_token = ctx->current;

  assert(ctx->current->type == TK_KEYWORD_TRY);
  const struct try_statement *try_statement_copy_opt =
      ctx->p_current_try_statement_opt;
...
struct selection_statement *owner selection_statement(struct parser_ctx *ctx) {
  ...
  */
  /*
     ...
     'switch' '(' expression ')' statement
  */
  struct selection_statement *owner p_selection_statement =
  ...
  if (ctx->current->type == TK_KEYWORD_IF) {
    ...
    if (first_of_declaration_specifier(ctx)) {
      struct declaration_specifiers *owner p_declaration_specifiers =
          move declaration_specifiers(ctx, STORAGE_SPECIFIER_AUTOMATIC_STORAGE);
      struct init_declarator_list list =
          init_declarator_list(ctx, p_declaration_specifiers, NULL);
      p_selection_statement->init_declarator = move list.head;
      parser_match_tk(ctx, ';');
    }
    ...
    if (constant_value_is_valid(
            &p_selection_statement->expression->constant_value)) {
    ...
    if (type_is_function(&p_selection_statement->expression->type) ||
      ...
      compiler_set_warning_with_token(W_ADDRESS, ctx, ctx->current,
                                      "always true");
    }
    ...
    if (ctx->current) {
      if (ctx->current->type == TK_KEYWORD_ELSE) {
        ...
        p_selection_statement->else_secondary_block_opt =
            move secondary_block(ctx);
      }
    } else {
      compiler_set_error_with_token(C_UNEXPECTED_END_OF_FILE, ctx,
                                    ctx->input_list.tail,
...
struct defer_statement *owner defer_statement(struct parser_ctx *ctx) {
  struct defer_statement *owner p_defer_statement =
      calloc(1, sizeof(struct defer_statement));
...
struct iteration_statement *owner iteration_statement(struct parser_ctx *ctx) {
  ...
  */
  struct iteration_statement *owner p_iteration_statement =
  ...
  if (ctx->current->type == TK_KEYWORD_DO) {
    ...
    if (first_of_declaration_specifier(ctx)) {
      ...
      p_iteration_statement->declaration =
          move declaration(ctx, move p_attribute_specifier_sequence_opt,
                           STORAGE_SPECIFIER_AUTOMATIC_STORAGE);
      if (ctx->current->type != ';') {
        p_iteration_statement->expression1 = move expression(ctx);
...
struct jump_statement *owner jump_statement(struct parser_ctx *ctx) {
  ...
  */
  ...
  struct jump_statement *owner p_jump_statement =
      calloc(1, sizeof(struct jump_statement));
  ...
  if (ctx->current->type == TK_KEYWORD_GOTO) {
    ...
    if (ctx->p_current_try_statement_opt == NULL) {
      ...
      compiler_set_error_with_token(C_THROW_STATEMENT_NOT_WITHIN_TRY_BLOCK, ctx,
                                    ctx->current,
                                    "throw statement not within try block");
    } else {
      p_jump_statement->try_catch_block_index =
          ctx->p_current_try_statement_opt->try_catch_block_index;
    }
    ...
    if (ctx->current->type != ';') {
      ...
      if (p_jump_statement->expression_opt) {
         ...
         */
        struct type return_type = get_function_return_type(
            &ctx->p_current_function_opt->init_declarator_list.head
                 ->p_declarator->type);

        if (type_is_void(&return_type)) {
          compiler_set_error_with_token(
              C_VOID_FUNCTION_SHOULD_NOT_RETURN_VALUE, ctx, p_return_token,
          ...
          if (p_jump_statement->expression_opt) {
            check_assigment(ctx, &return_type, p_jump_statement->expression_opt,
                            p_jump_statement->token_move != NULL, true);
          }
        }
...
struct expression_statement *owner
expression_statement(struct parser_ctx *ctx) {
...
void declaration_list_add(struct declaration_list *list,
                          struct declaration *owner p_declaration) {
  if (list->head == NULL) {
...
static void show_unused_file_scope(struct parser_ctx *ctx) {
  ...
  for (int i = 0; i < ctx->scopes.head->variables.capacity; i++) {
    ...
    while (entry) {
      ...
      if (p_declarator && p_declarator->first_token &&
          p_declarator->first_token->level == 0 &&
          declarator_is_function(p_declarator) &&
          (p_declarator->declaration_specifiers->storage_class_specifier_flags &
           STORAGE_SPECIFIER_STATIC)) {
        if (!type_is_maybe_unused(&p_declarator->type) &&
            p_declarator->num_uses == 0) {
          compiler_set_warning_with_token(
...
int fill_preprocessor_options(int argc, const char **argv,
...
const char *owner format_code(struct options *options, const char *content) {
  ...
  try {
    ...
    if (options->direct_compilation)
      ...
      s = move get_code_as_we_see(&visit_ctx.ast.token_list,
...
int compile_one_file(const char *file_name, struct options *options,
                     const char *out_file_name, int argc, const char **argv,
  ...
  try {
    ...
    struct token_list tokens =
        tokenizer(&tctx, content, file_name, 0, TK_FLAG_NONE);
...
static void longest_common_path(int argc, const char **argv,
...
struct ast get_ast(struct options *options, const char *filename,
                   const char *source, struct report *report) {
  struct ast ast = {0};
  struct tokenizer_ctx tctx = {0};
...
const char *owner compile_source(const char *pszoptions, const char *content,
                                 struct report *report) {
  ...
  try {
    ...
    if (options.preprocess_only) {
      struct tokenizer_ctx tctx = {0};
      struct token_list tokens =
          tokenizer(&tctx, content, "source", 0, TK_FLAG_NONE);
...
void naming_convention_struct_tag(struct parser_ctx *ctx, struct token *token) {
  ...
  if (ctx->options.style == STYLE_CAKE) {
    if (!is_snake_case(token->lexeme)) {
      compiler_set_info_with_token(W_STYLE, ctx, token,
                                   "use snake_case for struct/union tags");
    }
  } else if (ctx->options.style == STYLE_MICROSOFT) {
    if (!is_pascal_case(token->lexeme)) {
      compiler_set_info_with_token(W_STYLE, ctx, token,
                                   "use camelCase for struct/union tags");
    }
  }
}
...
void naming_convention_enum_tag(struct parser_ctx *ctx, struct token *token) {
  ...
  if (ctx->options.style == STYLE_CAKE) {
    if (!is_snake_case(token->lexeme)) {
      compiler_set_info_with_token(W_STYLE, ctx, token,
                                   "use snake_case for enum tags");
    }
  } else if (ctx->options.style == STYLE_MICROSOFT) {
    if (!is_pascal_case(token->lexeme)) {
      compiler_set_info_with_token(W_STYLE, ctx, token,
                                   "use PascalCase for enum tags");
    }
  }
}
  ...
  if (!type_is_function_or_function_pointer(type)) {
    if (storage & STORAGE_SPECIFIER_STATIC) {
      if (token->lexeme[0] != 's' || token->lexeme[1] != '_') {
        compiler_set_info_with_token(
            W_STYLE, ctx, token, "use prefix s_ for static global variables");
      }
    }
    if (!is_snake_case(token->lexeme)) {
      compiler_set_info_with_token(W_STYLE, ctx, token,
                                   "use snake_case global variables");
    }
  }
}
...
void take_address_type_test() {
  const char *src = "void F(char(*p)[10])"
                    "{"
                    "    (*p)[0] = 'a';"
                    "}";
  assert(compile_without_errors(src));
...
void sizeof_array_test() {
  assert(compile_without_errors("int main() {\n"
                                "int a[] = { 1, 2, 3 };\n"
...
void alignof_test() {
  const char *src = "struct X { char s; double c; char s2;};\n"
...
void indirection_struct_size() {
  const char *src = "typedef struct X X;\n"
                    "struct X {\n"
                    "    void* data;\n"
                    "};\n"
...
void expr_type() {
  const char *src = "static_assert(_is_same(typeof(1 + 2.0), double));";

  assert(compile_without_errors(src));
}

void expand_test() {
  char *src = "typedef int A[2];"
              "typedef A *B [1];"
...
void digit_separator_test() {
  const char *source = "static_assert(1'00'00 == 10000);";

  assert(compile_without_errors(source));
}

void numbers_test() {
  const char *source = "#if 0xA1 == 161\n"
...
void binary_digits_test() {
  const char *source = "_Static_assert(0b101010 == 42);"
...
void is_pointer_test() {
  const char *source =
      ...
      "  static_assert(_is_floating_point(double) && "
      "_is_floating_point(float));\n"
...
void simple_no_discard_test() {
  ...
  struct options options = {.input = LANGUAGE_C99,
                            .enabled_warnings_stack[0] = (~0 & ~W_STYLE)};
  struct report report = {0};
  get_ast(&options, "source", source, &report);
...
void return_address_of_local() {
  ...
  assert(report.warnings_count == 1 &&
         report.last_warning == W_RETURN_LOCAL_ADDR);
}
...
void simple_move() {
  const char *source = "char * _Owner f() {\n"
                       "    char * _Owner p = 0;\n"
...
void no_explicit_move_required() {
  const char *source = "char * _Owner create();\n"
...
void explicit_move_not_required() {
  const char *source = "#define NULL ((void*)0)\n"
                       "\n"
                       "int main()\n"
                       "{\n"
                       "    const char * _Owner s;\n"
...
void error_using_temporary_owner() {
  ...
  get_ast(&options, "source", source, &report);
  assert(report.error_count == 1 &&
         report.last_error == C_USING_TEMPORARY_OWNER);
}

void passing_view_to_owner() {
  const char *source = "void destroy(_Implicit _Owner int i);\n"
                       "\n"
                       "int main()\n"
                       "{\n"
                       "  _Owner int i = 0;\n"
...
void obj_owner_cannot_be_used_in_non_pointer() {
  ...
  get_ast(&options, "source", source, &report);
  assert(report.error_count == 1 &&
         report.last_error == C_OBJ_OWNER_CAN_BE_USED_ONLY_IN_POINTER);
}
...
void move_inside_if() {
  const char *source = "void free(_Implicit void* _Owner ptr);\n"
...
void no_warning() {
  const char *source = "void free(_Implicit void * _Owner p);\n"
                       "struct X {\n"
                       "  char * _Owner text;\n"
                       "};\n"
...
void moved_if_not_null() {
  const char *source = "void * _Owner malloc(int i);\n"
                       "void free(_Implicit void * _Owner p);\n"
                       "\n"
                       "struct X { int i; };\n"
