...
bool print_type_specifier_flags(
    struct osstream *ss, bool *first,
    enum type_specifier_flags e_type_specifier_flags) {
...
void print_type_qualifier_flags(
    struct osstream *ss, bool *first,
    enum type_qualifier_flags e_type_qualifier_flags) {
...
struct type type_lvalue_conversion(struct type *p_type) {
  ...
  case TYPE_CATEGORY_ARRAY: {
    /*
      ...
      to an expression with type "pointer to type" that points to the initial
      element of the array object and s not an lvalue. If the array object has
    ...
    */
    ...
    int g(const int a[const 20]) {
        // in this function, a has type const int* const (const pointer to const
    int)
        }
    */
    type_destroy(&t);
...
void print_type_core(struct osstream *ss, const struct type *p_type,
                     bool onlydeclarator) {
  ...
  if (p_type->struct_or_union_specifier) {
    ...
    */
    p_attribute_specifier_sequence_opt =
        p_type->struct_or_union_specifier->attribute_specifier_sequence_opt;
    ...
    p_attribute_specifier_sequence_opt =
        p_type->enum_specifier->attribute_specifier_sequence_opt;
...
/*
 ...
 There are three decimal floating types, designated as _Decimal32, _Decimal64,
 and _Decimal128.
*/
bool type_is_floating_point(const struct type *p_type) {
  ...
  return p_type->type_specifier_flags &
         (TYPE_SPECIFIER_DOUBLE | TYPE_SPECIFIER_FLOAT);
}
...
bool type_is_integer(const struct type *p_type) {
  ...
  return p_type->type_specifier_flags &
         (TYPE_SPECIFIER_CHAR | TYPE_SPECIFIER_SHORT | TYPE_SPECIFIER_INT |

          TYPE_SPECIFIER_INT16 | TYPE_SPECIFIER_INT32 | TYPE_SPECIFIER_INT64 |

          TYPE_SPECIFIER_INT | TYPE_SPECIFIER_LONG | TYPE_SPECIFIER_SIGNED |
          TYPE_SPECIFIER_UNSIGNED | TYPE_SPECIFIER_INT8 | TYPE_SPECIFIER_INT16 |
          TYPE_SPECIFIER_INT64 | TYPE_SPECIFIER_LONG_LONG |
          TYPE_SPECIFIER_BOOL);
}
...
bool type_is_scalar(const struct type *p_type) {
  // TODO we need two concepts...is_scalar on real type or is_scalar after
  // lvalue converison
...
type_get_func_or_func_ptr_params(const struct type *p_type) {
  if (p_type->category == TYPE_CATEGORY_FUNCTION) {
    ...
    if (p_type->next && p_type->next->category == TYPE_CATEGORY_FUNCTION) {
...
void check_function_argument_and_parameter(
  ...
  if (paramer_type->type_qualifier_flags & TYPE_QUALIFIER_OWNER ||
    ...
    if (current_argument->move_token == NULL &&
      ...
      if (current_argument->expression->type.storage_class_specifier_flags &
          STORAGE_SPECIFIER_FUNCTION_RETURN) {
        // function result does not need
      } else {
        compiler_set_warning_with_token(
            W_EXPLICIT_MOVE, ctx, current_argument->expression->first_token,
            "parameter %d requires explicit move", param_num);
      }

      if (paramer_type->type_qualifier_flags & TYPE_QUALIFIER_OWNER) {
        if (!(current_argument->expression->type.type_qualifier_flags &
              TYPE_QUALIFIER_OWNER)) {

          compiler_set_error_with_token(
              C_NOT_OWNER, ctx, current_argument->expression->first_token,
              "parameter %d requires a owner type", param_num);
        }
      }
    }

    if (paramer_type->type_qualifier_flags & TYPE_QUALIFIER_OBJ_OWNER) {
      if (current_argument->expression->type.category ==
          TYPE_CATEGORY_POINTER) {
        if (current_argument->expression->type.next &&
            !(current_argument->expression->type.next->type_qualifier_flags &
              TYPE_QUALIFIER_OWNER)) {
          compiler_set_error_with_token(
              C_NOT_OWNER, ctx, current_argument->expression->first_token,
              "parameter %d requires a pointer to owner object", param_num);
        }
      } else {
        compiler_set_error_with_token(
            C_NOT_OWNER, ctx, current_argument->expression->first_token,
            "parameter %d requires a pointer to owner type", param_num);
      }
    }
  }
  ...
  if (type_is_nullptr_t(&current_argument->expression->type) ||
      (constant_value_is_valid(&current_argument->expression->constant_value) &&
       constant_value_to_ull(&current_argument->expression->constant_value) ==
           0)) {
  ...
  */
  if (type_is_enum(argument_type) && type_is_enum(paramer_type)) {
    if (!type_is_same(argument_type, paramer_type, false)) {
      compiler_set_error_with_token(
          C_INCOMPATIBLE_TYPES, ctx, current_argument->expression->first_token,
  ...
  if (is_null_pointer_constant && type_is_array(paramer_type)) {
    compiler_set_warning_with_token(W_NON_NULL, ctx,
  ...
  */
  if (type_is_pointer_or_array(argument_type) &&
      type_is_pointer_or_array(paramer_type)) {
    ...
    if (type_is_array(paramer_type)) {
      ...
      if (type_is_array(argument_type)) {
        ...
        if (parameter_array_size != 0 &&
          ...
          compiler_set_error_with_token(
              C_ARGUMENT_SIZE_SMALLER_THAN_PARAMETER_SIZE, ctx,
              current_argument->expression->first_token,
              " argument of size [%d] is smaller than parameter of size [%d]",
              argument_array_size, parameter_array_size);
        }
    ...
    if (!type_is_same(&argument_type_converted, &parameter_type_converted,
                      false)) {
    ...
    if (type_is_pointer(&argument_type_converted) &&
        type_is_pointer(&parameter_type_converted)) {
      ...
      struct type parameter_pointer_to =
          type_remove_pointer(&parameter_type_converted);
      if (type_is_const(&argument_pointer_to) &&
        ...
        compiler_set_error_with_token(C_DISCARDING_CONST_AT_ARGUMENT, ctx,
                                      current_argument->expression->first_token,
                                      " discarding const at argument %d",
                                      param_num);
      }
  ...
  // TODO
  // if (!type_is_same(paramer_type, &current_argument->expression->type,
  // false))
  //{
...
continuation:
  ...
  if (paramer_type->type_qualifier_flags & TYPE_QUALIFIER_OWNER) {
    if (current_argument->expression->type.type_qualifier_flags &
      ...
      if (!is_null_pointer_constant) {
        compiler_set_error_with_token(
            C_MOVE_ASSIGNMENT_OF_NON_OWNER, ctx,
            current_argument->expression->first_token,
    ...
    if (paramer_type->type_qualifier_flags & TYPE_QUALIFIER_OWNER) {
      ...
      if (paramer_type->storage_class_specifier_flags &
          STORAGE_SPECIFIER_FUNCTION_RETURN) {
        // non owner = (owner) f()
        compiler_set_error_with_token(
            C_NON_OWNER_MOVE, ctx, current_argument->expression->first_token,
            "cannot move a temporary owner to non-owner");
      }

      if (explicit_move) {
        // non owner = _Move owner ERROR
        compiler_set_error_with_token(C_NON_OWNER_MOVE, ctx,
                                      current_argument->expression->first_token,
  ...
  //{
  //  compiler_set_error_with_token(C_MISSING_OWNER, ctx, right->first_token,
  //  "left type must be owner qualified ");
  //}
  //    }
  //}
...
void check_assigment(struct parser_ctx *ctx, struct type *left_type,
                     struct expression *right, bool explicit_move,
  ...
  if (!(right->type.type_qualifier_flags & TYPE_QUALIFIER_OWNER) &&
    ...
    if (!is_null_pointer_constant) {
      compiler_set_warning_with_token(W_NON_OWNER_ASSIGN, ctx,
                                      right->first_token,
  ...
  */
  if (type_is_enum(p_right_type) && type_is_enum(left_type)) {
    if (!type_is_same(p_right_type, left_type, false)) {
      compiler_set_error_with_token(C_INCOMPATIBLE_ENUN_TYPES, ctx,
                                    right->first_token, " incompatible types ");
    }
    goto continuation;
  }
  ...
  if (is_null_pointer_constant && type_is_array(left_type)) {
    compiler_set_warning_with_token(W_NON_NULL, ctx, right->first_token,
                                    " passing null as array");
  ...
  /*
     We have two pointers or pointer/array combination
  */
  if (type_is_pointer_or_array(p_right_type) &&
      type_is_pointer_or_array(left_type)) {
...
continuation:
  ...
  if (return_assignment) {
    if (type_is_pointer(&right->type) && right->type.next &&
        right->type.next->storage_class_specifier_flags &
            STORAGE_SPECIFIER_AUTOMATIC_STORAGE) {
      compiler_set_warning_with_token(
          W_RETURN_LOCAL_ADDR, ctx, right->first_token,
          "function returns address of local variable");
    }

    /*              return | non owner  | owner
        non owner          | OK         | if external, or param
        owner          |   | ERROR      | explicit if local, non explicit if
       external or param
    */
    ...
    if (right->type.type_qualifier_flags & TYPE_QUALIFIER_OWNER) {
      if (left_type->type_qualifier_flags & TYPE_QUALIFIER_OWNER) {
        ...
        if (right->type.storage_class_specifier_flags &
                STORAGE_SPECIFIER_PARAMETER ||
            right->type.storage_class_specifier_flags &
                STORAGE_SPECIFIER_EXTERN) {
          if (!explicit_move) {
            compiler_set_error_with_token(
                C_MOVE_ASSIGNMENT_OF_NON_OWNER, ctx, right->first_token,
    ...
    if (left_type->type_qualifier_flags & TYPE_QUALIFIER_OWNER) {
      ...
      if (right->type.type_qualifier_flags & TYPE_QUALIFIER_OWNER) {
        ...
        if (right->type.storage_class_specifier_flags &
          ...
          // non owner = (owner) f()
          compiler_set_error_with_token(
              C_NON_OWNER_MOVE, ctx, right->first_token,
              "cannot move a temporary owner to non-owner");
        }
...
bool type_is_pointer_or_array(const struct type *p_type) {
  ...
  if (category == TYPE_CATEGORY_POINTER || category == TYPE_CATEGORY_ARRAY)
    return true;
...
int type_get_struct_num_members(
    struct struct_or_union_specifier *complete_struct_or_union_specifier) {
  int count = 0;
  struct member_declaration *d =
      complete_struct_or_union_specifier->member_declaration_list.head;
  while (d) {
  ...
  if (category == TYPE_CATEGORY_POINTER) {
    ...
    if (p_type->type_specifier_flags & TYPE_SPECIFIER_CHAR) {
      ...
      if (p_complete) {
        ...
        align = -2;
        // seterror(error, "invalid application of 'sizeof' to incomplete type
        // 'struct %s'", p_type->struct_or_union_specifier->tag_name);
      }
    } else if (p_type->type_specifier_flags & TYPE_SPECIFIER_ENUM) {
      align = _Alignof(int);
...
unsigned int type_get_hashof(struct parser_ctx *ctx, struct type *p_type) {
  ...
  if (type_is_struct_or_union(p_type)) {
    ...
    if (p_complete) {
      ...
      for (; current != p_complete->last_token->next; current = current->next) {
...
struct type type_get_enum_type(const struct type *p_type) {
  ...
  if (p_type->enum_specifier->complete_enum_specifier &&
    ...
    struct type t = {0};
    t.type_qualifier_flags =
        p_type->enum_specifier->complete_enum_specifier
            ->specifier_qualifier_list->type_qualifier_flags;
    t.type_specifier_flags =
        p_type->enum_specifier->complete_enum_specifier
            ->specifier_qualifier_list->type_specifier_flags;
    return t;
  }
...
bool struct_or_union_specifier_is_same(struct struct_or_union_specifier *a,
                                       struct struct_or_union_specifier *b) {
  if (a && b) {
...
bool enum_specifier_is_same(struct enum_specifier *a,
                            struct enum_specifier *b) {
  if (a && b) {
...
bool type_is_same(const struct type *a, const struct type *b,
  ...
  while (pa && pb) {
    ...
    if (pa->array_size != pb->array_size)
      return false;
    ...
    if (pa->struct_or_union_specifier && pb->struct_or_union_specifier) {
      ...
      if (strcmp(pa->struct_or_union_specifier->tag_name,
                 pb->struct_or_union_specifier->tag_name) != 0) {
        return false;
      }
    }

    if (compare_qualifiers &&
        pa->type_qualifier_flags != pb->type_qualifier_flags) {
      return false;
    }
...
void type_visit_to_mark_anonymous(struct type *p_type) {
  ...
  if (p_type->struct_or_union_specifier != NULL &&
    ...
    if (p_type->struct_or_union_specifier
            ->complete_struct_or_union_specifier_indirection) {
      p_type->struct_or_union_specifier
          ->complete_struct_or_union_specifier_indirection->show_anonymous_tag =
          true;
    }
...
void type_set_storage_specifiers_using_declarator(
    struct type *p_type, struct declarator *pdeclarator) {
...
void type_set_specifiers_using_declarator(struct type *p_type,
                                          struct declarator *pdeclarator) {
  if (pdeclarator->declaration_specifiers) {
...
void type_set_attributes_using_declarator(struct type *p_type,
                                          struct declarator *pdeclarator) {
  if (pdeclarator->declaration_specifiers) {
    if (pdeclarator->declaration_specifiers->attributes_flags &
        STD_ATTRIBUTE_NODISCARD) {
      p_type->storage_class_specifier_flags |=
...
void type_list_push_front(struct type_list *books,
                          struct type *owner new_book) {
  assert(books != NULL);
...
void make_type_using_declarator_core(struct parser_ctx *ctx,
                                     struct declarator *pdeclarator,
                                     char **ppname, struct type_list *list);

void make_type_using_direct_declarator(
    struct parser_ctx *ctx, struct direct_declarator *pdirectdeclarator,
    char **ppname, struct type_list *list) {
  if (pdirectdeclarator->declarator) {
    make_type_using_declarator_core(ctx, pdirectdeclarator->declarator, ppname,
                                    list);
  }

  else if (pdirectdeclarator->function_declarator) {
    if (pdirectdeclarator->function_declarator->direct_declarator) {
      make_type_using_direct_declarator(
          ctx, pdirectdeclarator->function_declarator->direct_declarator,
          ppname, list);
    }
    ...
    if (pdirectdeclarator->function_declarator->parameter_type_list_opt &&
        pdirectdeclarator->function_declarator->parameter_type_list_opt
            ->parameter_list) {
      ...
      p_func->params.is_var_args = pdirectdeclarator->function_declarator
                                       ->parameter_type_list_opt->is_var_args;
      p_func->params.is_void = pdirectdeclarator->function_declarator
                                   ->parameter_type_list_opt->is_void;
    ...
    if (pdirectdeclarator->array_declarator->direct_declarator) {
      make_type_using_direct_declarator(
          ctx, pdirectdeclarator->array_declarator->direct_declarator, ppname,
          list);
    }
    ...
    if (pdirectdeclarator->array_declarator->type_qualifier_list_opt) {
      p->type_qualifier_flags =
          pdirectdeclarator->array_declarator->type_qualifier_list_opt->flags;
    }
...
void make_type_using_declarator_core(struct parser_ctx *ctx,
  ...
  while (pointer) {
    ...
    if (pointer->attribute_specifier_sequence_opt) {
      p_flat->attributes_flags |=
          pointer->attribute_specifier_sequence_opt->attributes_flags;
    }
  ...
  if (pdeclarator->direct_declarator) {
    make_type_using_direct_declarator(ctx, pdeclarator->direct_declarator,
                                      ppname, list);
    if (list->head && list->head->category == TYPE_CATEGORY_FUNCTION) {
      if (pointers.head) {
        pointers.head->storage_class_specifier_flags |=
            STORAGE_SPECIFIER_FUNCTION_RETURN;
      }
    }
  }
...
declarator_get_enum_specifier(struct declarator *pdeclarator) {
  if (pdeclarator->declaration_specifiers &&
      pdeclarator->declaration_specifiers->enum_specifier) {
...
declarator_get_struct_or_union_specifier(struct declarator *pdeclarator) {
  if (pdeclarator->declaration_specifiers &&
      pdeclarator->declaration_specifiers->struct_or_union_specifier) {
...
declarator_get_typeof_specifier(struct declarator *pdeclarator) {
  if (pdeclarator->declaration_specifiers) {
...
struct declarator *
declarator_get_typedef_declarator(struct declarator *pdeclarator) {
  if (pdeclarator->declaration_specifiers) {
...
struct type make_type_using_declarator(struct parser_ctx *ctx,
  ...
  if (declarator_get_typeof_specifier(pdeclarator)) {
    ...
    *p_nt = move nt;
    ...
    if (list.tail && list.tail->category == TYPE_CATEGORY_FUNCTION) {
