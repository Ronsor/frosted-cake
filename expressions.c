...
bool constant_value_is_valid(const struct constant_value *a) {
  return a->type != TYPE_NOT_CONSTANT && a->type != TYPE_EMPTY;
}

void constant_value_to_string(const struct constant_value *a, char buffer[],
                              int sz) {
  buffer[0] = 0;
  switch (a->type) {
  case TYPE_LONG_LONG:
...
struct constant_value constant_value_cast(const struct constant_value *a,
...
struct constant_value constant_value_unary_op(const struct constant_value *a,
                                              int op) {
...
struct constant_value constant_value_op(const struct constant_value *a,
                                        const struct constant_value *b,
                                        int op) {
  ...
  if (a->type == TYPE_UNSIGNED_LONG_LONG ||
      b->type == TYPE_UNSIGNED_LONG_LONG) {
...
static int compare_function_arguments(
    struct parser_ctx *ctx, struct type *p_type,
    struct argument_expression_list *p_argument_expression_list) {
  try {
    struct param *p_current_parameter_type = NULL;

    const struct param_list *p_param_list =
        type_get_func_or_func_ptr_params(p_type);
    ...
    while (p_current_argument && p_current_parameter_type) {
      check_function_argument_and_parameter(
          ctx, p_current_argument, &p_current_parameter_type->type, param_num);
    ...
    if (p_current_parameter_type != NULL && !p_param_list->is_void) {
      if (p_argument_expression_list->tail) {
        compiler_set_error_with_token(
            C_TOO_FEW_ARGUMENTS, ctx,
            p_argument_expression_list->tail->expression->first_token,
            "too few arguments");
      } else {
        compiler_set_error_with_token(C_TOO_FEW_ARGUMENTS, ctx, ctx->current,
                                      "too few arguments");
      }
      throw;
    }
...
bool is_enumeration_constant(struct parser_ctx *ctx) {
  ...
  const bool is_enumerator =
      find_enumerator(ctx, ctx->current->lexeme, NULL) != NULL;
...
bool is_first_of_constant(struct parser_ctx *ctx) {
  ...
  */
  return is_first_of_integer_constant(ctx) ||
         is_first_of_floating_constant(ctx) || is_enumeration_constant(ctx) ||
...
bool is_first_of_primary_expression(struct parser_ctx *ctx) {
  ...
  */
  ...
  return ctx->current->type == TK_IDENTIFIER || is_first_of_constant(ctx) ||
         ctx->current->type == TK_STRING_LITERAL || ctx->current->type == '(' ||
...
struct generic_association *owner generic_association(struct parser_ctx *ctx) {
  ...
  try {
    ...
    if (ctx->current && ctx->current->type == TK_KEYWORD_DEFAULT) {
      ...
      p_generic_association->type = move make_type_using_declarator(
          ctx, p_generic_association->p_type_name->declarator);
    } else {
...
struct generic_assoc_list generic_association_list(struct parser_ctx *ctx) {
  ...
  try {
    ...
    while (ctx->current->type == ',') {
      ...
      struct generic_association *owner p_generic_association2 =
...
struct generic_selection *owner generic_selection(struct parser_ctx *ctx) {
  ...
  */
  ...
  try {
    ...
    p_generic_selection->generic_assoc_list =
        move generic_association_list(ctx);
    ...
    if (p_generic_selection->expression) {
      ...
      if (expression_is_subjected_to_lvalue_conversion(
              p_generic_selection->expression)) {
        lvalue_type =
            move type_lvalue_conversion(&p_generic_selection->expression->type);
        p_type = &lvalue_type;
      }
    ...
    struct generic_association *current =
        p_generic_selection->generic_assoc_list.head;
    while (current) {
...
// enum token_type parse_number(const char* lexeme, enum type_specifier_flags*
// flags);

int convert_to_number(struct token *token,
  ...
  case TK_COMPILER_DECIMAL_CONSTANT:
    ...
    if (flags & TYPE_SPECIFIER_UNSIGNED) {
      p_expression_node->constant_value =
          make_constant_value_ull(strtoull(buffer, 0, 10));
    } else {
      p_expression_node->constant_value =
          make_constant_value_ll(strtoll(buffer, 0, 10));
    }

    break;
  case TK_COMPILER_OCTAL_CONSTANT:
    p_expression_node->constant_value =
        make_constant_value_ll(strtoll(buffer, 0, 8));

    break;
  case TK_COMPILER_HEXADECIMAL_CONSTANT:
    p_expression_node->constant_value =
        make_constant_value_ll(strtoll(buffer + 2, 0, 16));

    break;
  case TK_COMPILER_BINARY_CONSTANT:
    p_expression_node->constant_value =
        make_constant_value_ll(strtoll(buffer + 2, 0, 2));
    break;
  case TK_COMPILER_DECIMAL_FLOATING_CONSTANT:
    p_expression_node->constant_value =
        make_constant_value_double(strtod(buffer, 0));
    break;
  case TK_COMPILER_HEXADECIMAL_FLOATING_CONSTANT:
    p_expression_node->constant_value =
        make_constant_value_double(strtod(buffer + 2, 0));
    break;
  default:
    assert(false);
  }
...
struct object *expression_get_object(struct expression *p_expression,
                                     struct type *p_type) {
  if (p_expression->expression_type == PRIMARY_EXPRESSION_DECLARATOR) {
    *p_type = type_dup(&p_expression->declarator->type);
    ...
    if (p_expression->left &&
        p_expression->left->expression_type == PRIMARY_EXPRESSION_DECLARATOR) {
    ...
    if (p_expression->left->declarator) {
      *p_type = type_dup(&p_expression->type);
      return &p_expression->left->declarator->object.members
                  .data[p_expression->member_index];
    }
    ...
    if (p_expression->left->declarator) {
      if (p_expression->left->declarator->object.pointed &&
        ...
        *p_type = type_dup(&p_expression->type);
        return &p_expression->left->declarator->object.pointed->members
                    .data[p_expression->member_index];
      }
    }
  }
  ...
  try {
    if (ctx->current->type == TK_IDENTIFIER) {
      ...
      struct map_entry *p_entry =
          find_variables(ctx, ctx->current->lexeme, NULL);
      ...
      if (p_entry && p_entry->type == TAG_TYPE_ENUMERATOR) {
        ...
        p_expression_node->constant_value =
            make_constant_value_ll(p_enumerator->value);

        p_expression_node->type =
            move type_make_enumerator(p_enumerator->enum_specifier);

      } else if (p_entry && (p_entry->type == TAG_TYPE_ONLY_DECLARATOR ||
                             p_entry->type == TAG_TYPE_INIT_DECLARATOR)) {
        ...
        if (type_is_deprecated(&p_declarator->type)) {
          compiler_set_warning_with_token(W_DEPRECATED, ctx, ctx->current,
                                          "'%s' is deprecated",
                                          ctx->current->lexeme);
        }
        ...
        /*
           ...
           works for now
        */
        const char *funcname = ctx->p_current_function_opt->init_declarator_list
                                   .head->p_declarator->name->lexeme;
        ...
        p_expression_node->type = move type_make_literal_string(
            strlen(funcname) + 1, TYPE_SPECIFIER_CHAR);
      } else {
        compiler_set_error_with_token(C_NOT_FOUND, ctx, ctx->current,
                                      "not found '%s'", ctx->current->lexeme);
        throw;
      }
      parser_match(ctx);
      ...
      p_expression_node->type = move type_make_literal_string(
          string_literal_byte_size(ctx->current->lexeme), char_type);
      ...
      */
      while (ctx->current && ctx->current->type == TK_STRING_LITERAL) {
        parser_match(ctx);
      }
      ...
      p_expression_node->type.attributes_flags |=
          CAKE_HIDDEN_ATTRIBUTE_LIKE_CHAR;
      ...
      p_expression_node->expression_type =
          PRIMARY_EXPRESSION_PREDEFINED_CONSTANT;
      p_expression_node->first_token = ctx->current;
      p_expression_node->last_token = ctx->current;
      ...
      p_expression_node->last_token =
          p_expression_node->generic_selection->last_token;

      if (p_expression_node->generic_selection->p_view_selected_expression) {
        p_expression_node->type =
            move type_dup(&p_expression_node->generic_selection
                               ->p_view_selected_expression->type);

        p_expression_node->constant_value =
            p_expression_node->generic_selection->p_view_selected_expression
                ->constant_value;
      } else {
        compiler_set_error_with_token(C_NO_MATCH_FOR_GENERIC, ctx, ctx->current,
                                      "no match for generic");
      }
...
argument_expression_list(struct parser_ctx *ctx) {
  /*
  ...
  */
  ...
  try {
    ...
    while (ctx->current->type == ',') {
      ...
      struct argument_expression *owner p_argument_expression_2 =
          calloc(1, sizeof *p_argument_expression_2);
...
static void fix_member_type(struct type *p_type, const struct type *struct_type,
  ...
  */
  p_type->storage_class_specifier_flags =
      struct_type->storage_class_specifier_flags;
...
static void fix_arrow_member_type(struct type *p_type, const struct type *left,
                                  const struct type *member_type) {
...
struct expression *owner postfix_expression_tail(
    struct parser_ctx *ctx, struct expression *owner p_expression_node) {
  try {
    while (ctx->current != NULL) {
      if (ctx->current->type == '[') {
        struct expression *owner p_expression_node_new =
            calloc(1, sizeof *p_expression_node_new);
        ...
        if (!type_is_pointer_or_array(&p_expression_node->type)) {
          compiler_set_error_with_token(
              C_SUBSCRIPTED_VALUE_IS_NEITHER_ARRAY_NOR_POINTER, ctx,
              ctx->current, "subscripted value is neither array nor pointer");
        }

        if (type_is_pointer(&p_expression_node->type)) {
          p_expression_node_new->type =
              move type_remove_pointer(&p_expression_node->type);

        } else if (type_is_array(&p_expression_node->type)) {
          p_expression_node_new->type =
              move get_array_item_type(&p_expression_node->type);
        }
        ...
        if (!type_is_function_or_function_pointer(&p_expression_node->type)) {
          compiler_set_error_with_token(
              C_CALLED_OBJECT_IS_NOT_FUNCTION_OR_FUNCTION_POINTER, ctx,
              ctx->current,
        ...
        p_expression_node_new->type =
            move get_function_return_type(&p_expression_node->type);

        parser_match(ctx);
        if (ctx->current->type != ')') {
          p_expression_node_new->argument_expression_list =
              move argument_expression_list(ctx);
        }
        parser_match_tk(ctx, ')');
        compare_function_arguments(
            ctx, &p_expression_node->type,
            &p_expression_node_new->argument_expression_list);
        ...
        if (p_expression_node_new->left->expression_type == POSTFIX_DOT) {
          p_expression_node_new->declarator =
              p_expression_node_new->left->declarator;
        } else {
          p_expression_node_new->declarator =
              p_expression_node_new->left->declarator;
          // assert(false);
          // int mindex = 0;
          // p_expression_node_new->declarator =
          // expression_get_declarator(p_expression_node, &mindex);
        }

        // struct declarator* declarator;

        parser_match(ctx);
        if (p_expression_node->type.type_specifier_flags &
            TYPE_SPECIFIER_STRUCT_OR_UNION) {
          struct struct_or_union_specifier *p = find_struct_or_union_specifier(
              ctx, p_expression_node->type.struct_or_union_specifier->tag_name);
          p = get_complete_struct_or_union_specifier(p);
          if (p) {
            int member_index = 0;
            struct member_declarator *p_member_declarator =
                find_member_declarator(&p->member_declaration_list,
                                       ctx->current->lexeme, &member_index);
            if (p_member_declarator) {
              if (p_expression_node_new->left->expression_type == POSTFIX_DOT) {
                p_expression_node_new->member_index =
                    member_index + p_expression_node_new->left->member_index;
              } else {
                p_expression_node_new->member_index = member_index;
              }

              p_expression_node_new->type = move make_type_using_declarator(
                  ctx, p_member_declarator->declarator);
              ...
              compiler_set_error_with_token(
                  C_STRUCT_MEMBER_NOT_FOUND, ctx, ctx->current,
                  "struct member '%s' not found in '%s'", ctx->current->lexeme,
          ...
          compiler_set_error_with_token(C_STRUCTURE_OR_UNION_REQUIRED, ctx,
                                        ctx->current,
        ...
        if (type_is_pointer_or_array(&p_expression_node->type)) {
          ...
          if (type_is_array(&p_expression_node->type)) {
            compiler_set_info_with_token(
                W_STYLE, ctx, ctx->current,
                "using '->' in array as pointer to struct");
          ...
          if (type_is_struct_or_union(&item_type)) {
            struct struct_or_union_specifier *p_complete =
                get_complete_struct_or_union_specifier(
                    p_expression_node->type.next->struct_or_union_specifier);

            if (p_complete) {
              int member_index = 0;
              struct member_declarator *p_member_declarator =
                  find_member_declarator(&p_complete->member_declaration_list,
                                         ctx->current->lexeme, &member_index);

              if (p_member_declarator) {
                p_expression_node_new->type = move make_type_using_declarator(
                    ctx, p_member_declarator->declarator);
                fix_arrow_member_type(&p_expression_node_new->type,
                                      &p_expression_node_new->left->type,
                                      &p_expression_node_new->type);
              } else {
                compiler_set_error_with_token(
              ...
              compiler_set_error_with_token(
                  C_STRUCT_IS_INCOMPLETE, ctx, ctx->current,
                  "struct '%s' is incomplete.", ctx->current->lexeme);
            }
            parser_match_tk(ctx, TK_IDENTIFIER);
...
struct expression *owner postfix_expression_type_name(
    struct parser_ctx *ctx, struct type_name *owner p_type_name) {
  /*
      ( type-name ) { initializer-ctx }
      ( type-name ) { initializer-ctx , }

      //My extension : if type-name is function then follow is
     compound-statement ( type-name ) compound-statement
  ...
  */
  ...
  try {
    ...
    p_expression_node->first_token =
        previous_parser_token(p_type_name->first_token);
    assert(p_expression_node->first_token->type == '(');

    p_expression_node->type_name = move p_type_name;
    p_expression_node->type = move make_type_using_declarator(
        ctx, p_expression_node->type_name->declarator);
    ...
    if (type_is_function(&p_type_name->declarator->type)) {
      ...
      struct scope *parameters_scope =
          &p_expression_node->type_name->declarator->direct_declarator
               ->function_declarator->parameters_scope;
    ...
    p_expression_node =
        move postfix_expression_tail(ctx, move p_expression_node);
    if (p_expression_node == NULL)
      throw;
  ...
  try {
    if (first_of_type_name_ahead(ctx)) // aqui preciso ver se nao eh primary
    {
      ...
      if (is_function_type) {
        ...
        p_expression_node->last_token =
            p_expression_node->compound_statement->last_token;
      } else {
        p_expression_node->expression_type =
...
bool is_first_of_unary_expression(struct parser_ctx *ctx) {
  if (ctx->current == NULL)
    return false;
  return first_of_postfix_expression(ctx) || ctx->current->type == '++' ||
         ctx->current->type == '--' || ctx->current->type == '&' ||
         ctx->current->type == '*' || ctx->current->type == '+' ||
         ctx->current->type == '-' || ctx->current->type == '~' ||
         ctx->current->type == '!' || ctx->current->type == TK_KEYWORD_SIZEOF ||
...
struct expression *owner unary_expression(struct parser_ctx *ctx) {
  ...
  try {
    if (ctx->current->type == '++' || ctx->current->type == '--') {
      struct expression *owner new_expression =
          calloc(1, sizeof *new_expression);
      ...
      if (op == '!') {
        ...
        new_expression->constant_value =
            constant_value_unary_op(&new_expression->right->constant_value, op);
        ...
        new_expression->type = move type_dup(&new_expression->right->type);
        new_expression->type.storage_class_specifier_flags &=
            ~STORAGE_SPECIFIER_LVALUE;
      } else if (op == '-') {
        ...
        new_expression->type =
            move type_remove_pointer(&new_expression->right->type);
      } else if (op == '&') {
        /*
          The result of the unary & operator is a pointer to the object referred
          to by the lvalue-expression. If the type of the lvalue-expression is "
        ...
        */
        ...
        if (!(new_expression->right->type.storage_class_specifier_flags &
              STORAGE_SPECIFIER_LVALUE)) {
        ...
        if (new_expression->right->type.storage_class_specifier_flags &
          ...
          if (new_expression->right->declarator) {
            compiler_set_error_with_token(
                C_ADDRESS_OF_REGISTER, ctx, new_expression->right->first_token,
        ...
        new_expression->type =
            move type_add_pointer(&new_expression->right->type);
      } else {
        compiler_set_error_with_token(C_INVALID_TOKEN, ctx, ctx->current,
                                      "invalid token");
        throw;
      }
      p_expression_node = move new_expression;
      ...
      if (first_of_type_name_ahead(ctx)) {
        ...
        new_expression->constant_value = make_constant_value_ll(
            type_get_sizeof(&new_expression->type_name->declarator->type));
        ...
        new_expression->constant_value = make_constant_value_ll(
            type_get_sizeof(&new_expression->right->type));
      }

      new_expression->type = move type_make_size_t();
      p_expression_node = move new_expression;
    } else if (ctx->current->type == TK_KEYWORD_IS_LVALUE ||
               ctx->current->type == TK_KEYWORD_IS_OWNER ||
               ctx->current->type == TK_KEYWORD_IS_CONST ||
               ctx->current->type == TK_KEYWORD_IS_POINTER ||
               ctx->current->type == TK_KEYWORD_IS_ARRAY ||
      ...
      case TK_KEYWORD_IS_LVALUE:
        new_expression->constant_value =
            make_constant_value_ll(type_is_lvalue(p_type));
        break;

      case TK_KEYWORD_IS_CONST:
        new_expression->constant_value =
            make_constant_value_ll(type_is_const(p_type));
        break;
      case TK_KEYWORD_IS_OWNER:
        new_expression->constant_value =
            make_constant_value_ll(type_is_owner(p_type));
        break;

      case TK_KEYWORD_IS_POINTER:
        new_expression->constant_value =
            make_constant_value_ll(type_is_pointer(p_type));

        break;
      case TK_KEYWORD_IS_FUNCTION:
        new_expression->constant_value =
            make_constant_value_ll(type_is_function(p_type));

        break;
      case TK_KEYWORD_IS_ARRAY:
        new_expression->constant_value =
            make_constant_value_ll(type_is_array(p_type));

        break;
      case TK_KEYWORD_IS_ARITHMETIC:
        new_expression->constant_value =
            make_constant_value_ll(type_is_arithmetic(p_type));

        break;
      case TK_KEYWORD_IS_SCALAR:
        new_expression->constant_value =
            make_constant_value_ll(type_is_scalar(p_type));

        break;
      case TK_KEYWORD_IS_FLOATING_POINT:
        new_expression->constant_value =
            make_constant_value_ll(type_is_floating_point(p_type));

        break;
      case TK_KEYWORD_IS_INTEGRAL:
        new_expression->constant_value =
            make_constant_value_ll(type_is_integer(p_type));
      ...
      // new_expression->declarator =
      // expression_get_declarator(new_expression->right,
      // &new_expression->member_index);
      ...
      parser_match_tk(ctx, '(');
      new_expression->type_name = move type_name(ctx);
      new_expression->type = move make_type_using_declarator(
          ctx, new_expression->type_name->declarator);
      parser_match_tk(ctx, ')');

      new_expression->constant_value =
          make_constant_value_ll(type_get_alignof(&new_expression->type));
  ...
  */
  ...
  try {
    if (first_of_type_name_ahead(ctx)) {
      ...
      p_expression_node->type =
          move type_dup(&p_expression_node->type_name->type);
      ...
      if (ctx->current->type == '{') {
        ...
        struct expression *owner new_expression = postfix_expression_type_name(
            ctx, move p_expression_node->type_name);
        ...
        if (p_expression_node->left == NULL)
          throw;
        p_expression_node->constant_value =
            p_expression_node->left->constant_value;
        p_expression_node->type = move make_type_using_declarator(
            ctx, p_expression_node->type_name->declarator);
        if (type_is_floating_point(&p_expression_node->type)) {
          p_expression_node->constant_value = constant_value_cast(
              &p_expression_node->constant_value, TYPE_DOUBLE);
        } else if (type_is_integer(&p_expression_node->type)) {
          if (type_is_unsigned_integer(&p_expression_node->type)) {
            p_expression_node->constant_value = constant_value_cast(
                &p_expression_node->constant_value, TYPE_UNSIGNED_LONG_LONG);
          } else {
            p_expression_node->constant_value = constant_value_cast(
                &p_expression_node->constant_value, TYPE_LONG_LONG);
          }
        }
      } else {
        p_expression_node->type = move make_type_using_declarator(
    ...
    while (ctx->current != NULL &&
      ...
      if (op == '*') {
        ...
        if (!type_is_arithmetic(&new_expression->left->type)) {
          compiler_set_error_with_token(C_LEFT_IS_NOT_ARITHMETIC, ctx,
                                        ctx->current,
                                        "left * is not arithmetic");
        }
        if (!type_is_arithmetic(&new_expression->right->type)) {
          compiler_set_error_with_token(C_RIGHT_IS_NOT_ARITHMETIC, ctx,
                                        ctx->current,
                                        "right * is not arithmetic");
        }
        ...
        new_expression->constant_value =
            constant_value_op(&new_expression->left->constant_value,
                              &new_expression->right->constant_value, '/');

        if (constant_value_is_valid(&new_expression->right->constant_value) &&
            constant_value_to_ll(&new_expression->right->constant_value) == 0) {
          compiler_set_error_with_token(C_DIVIZION_BY_ZERO, ctx, ctx->current,
                                        "divizion by zero");
        }
        ...
        if (!type_is_integer(&new_expression->left->type)) {
          compiler_set_error_with_token(C_LEFT_IS_NOT_INTEGER, ctx,
                                        ctx->current, "left is not integer");
        }
        if (!type_is_integer(&new_expression->right->type)) {
          compiler_set_error_with_token(C_RIGHT_IS_NOT_INTEGER, ctx,
                                        ctx->current, "right is not integer");
        }
      ...
      new_expression->constant_value =
          constant_value_op(&new_expression->left->constant_value,
                            &new_expression->right->constant_value, op);

      int code =
          type_common(&new_expression->left->type, &new_expression->right->type,
                      &new_expression->type);
      if (code != 0) {
        compiler_set_error_with_token(C_INVALID_TYPE, ctx, ctx->current,
                                      "invalid type multiplicative expression");
        throw;
      }
  ...
  try {
    ...
    while (ctx->current != NULL &&
      ...
      if (!type_is_scalar(&new_expression->left->type)) {
        compiler_set_error_with_token(C_LEFT_IS_NOT_SCALAR, ctx,
                                      operator_position,
                                      "left operator is not scalar");
      }
      if (!type_is_scalar(&new_expression->right->type)) {
        compiler_set_error_with_token(C_RIGHT_IS_NOT_SCALAR, ctx,
                                      operator_position,
                                      "right operator is not scalar");
      }

      const bool b_left_is_arithmetic =
          type_is_arithmetic(&new_expression->left->type);
      const bool b_right_is_arithmetic =
          type_is_arithmetic(&new_expression->right->type);

      const enum type_category left_category =
          type_get_category(&new_expression->left->type);
      const enum type_category right_category =
          type_get_category(&new_expression->right->type);
      ...
      if (op == '+') {
        ...
        /*
         ...
         the other shall have integer type. (Incrementing is equivalent to
         adding 1.)
        */
        if (b_left_is_arithmetic && b_right_is_arithmetic) {
          int code =
              type_common(&new_expression->left->type,
                          &new_expression->right->type, &new_expression->type);
          if (code != 0) {
            compiler_set_error_with_token(C_UNEXPECTED, ctx, ctx->current,
                                          "internal error");
            throw;
          ...
          if (left_category == TYPE_CATEGORY_POINTER || left_category ==
                                                            TYPE_CATEGORY_ARRAY /* || left_category == TYPE_CATEGORY_FUNCTION*/) {
            if (type_is_integer(&new_expression->right->type)) {
              if (left_category == TYPE_CATEGORY_ARRAY) {
                struct type t =
                    get_array_item_type(&new_expression->left->type);
                new_expression->type = move type_add_pointer(&t);
                type_destroy(&t);
              } else {
                new_expression->type =
                    move type_dup(&new_expression->left->type);
              }
            } else {
              compiler_set_error_with_token(C_RIGHT_IS_NOT_INTEGER, ctx,
                                            ctx->current,
                                            "expected integer type on right");
            }
          } else if (
              right_category == TYPE_CATEGORY_POINTER || right_category ==
                                                             TYPE_CATEGORY_ARRAY /*|| right_category == TYPE_CATEGORY_FUNCTION*/) {
            if (type_is_integer(&new_expression->left->type)) {
              if (right_category == TYPE_CATEGORY_ARRAY) {
                new_expression->type =
                    move get_array_item_type(&new_expression->right->type);
              } else {
                new_expression->type =
                    move type_dup(&new_expression->right->type);
              }
            } else {
              compiler_set_error_with_token(C_LEFT_IS_NOT_INTEGER, ctx,
                                            ctx->current,
                                            "expected integer type on left");
            }
          } else {
            compiler_set_error_with_token(C_INVALID_TYPE, ctx, ctx->current,
                                          "invalid types additive expression");
          }
        }
      } else if (op == '-') {
        ...
        /*
         ...
         compatible complete object types; or — the left operand is a pointer to
         a complete object type and the right operand has integer type.
        ...
        */
        if (b_left_is_arithmetic && b_right_is_arithmetic) {
          ...
          if (code != 0) {
            compiler_set_error_with_token(C_INVALID_TYPE, ctx, ctx->current,
                                          "internal error type_common");
            throw;
          }
        } else {
          if (left_category == TYPE_CATEGORY_POINTER ||
              left_category == TYPE_CATEGORY_ARRAY) {
            if (right_category == TYPE_CATEGORY_POINTER ||
                right_category == TYPE_CATEGORY_ARRAY) {
              // — both operands are pointers to qualified or unqualified
              // versions of compatible complete object
              //  types;
              struct type t1 =
                  type_lvalue_conversion(&new_expression->left->type);
              struct type t2 =
                  type_lvalue_conversion(&new_expression->right->type);

              if (!type_is_same(&t1, &t2, false)) {
                compiler_set_error_with_token(C_INCOMPATIBLE_POINTER_TYPES, ctx,
                                              ctx->current,
              ...
              if (type_is_integer(&new_expression->right->type)) {
                //- the left operand is a pointer to a complete object typeand
  ...
  try {
    ...
    while (ctx->current != NULL &&
      ...
      if (op == '>') {
        ...
        new_expression->expression_type =
...
static void check_diferent_enuns(struct parser_ctx *ctx,
  ...
  if (left->type.type_specifier_flags & TYPE_SPECIFIER_ENUM &&
    ...
    if (left->type.enum_specifier->complete_enum_specifier !=
      ...
      compiler_set_warning_with_token(
          W_ENUN_COMPARE, ctx, operator_token,
          "comparison between 'enum %s' and 'enum %s'", lefttag, righttag);
    }
  }
}
...
struct expression *owner equality_expression(struct parser_ctx *ctx) {
  ...
  * Equality operators
  ...
  — both operands have arithmetic type;
  — both operands are pointers to qualified or unqualified versions of
  compatible types; — one operand is a pointer to an object type and the other
  is a pointer to a qualified or unqualified version of void; or — one operand
  is a pointer and the other is a null pointer constant.
  */
  struct expression *owner p_expression_node = NULL;
  struct expression *owner new_expression = NULL;
  ...
  try {
    ...
    while (ctx->current != NULL &&
      ...
      check_diferent_enuns(ctx, operator_token, new_expression->left,
      ...
      if (operator_token->type == '==') {
        ...
        if (new_expression->left->constant_value.type == TYPE_EMPTY ||
          ...
          new_expression->constant_value = make_constant_value_ll(type_is_same(
              &new_expression->left->type, &new_expression->right->type, true));
        }
  ...
  */
  ...
  try {
    ...
    while (ctx->current != NULL && (ctx->current->type == '|')) {
      ...
      if (code != 0) {
        compiler_set_error_with_token(C_INVALID_TYPE, ctx, ctx->current,
                                      "invalid types inclusive or expression");
        throw;
      }
      p_expression_node = move new_expression;
    }
  ...
  try {
    ...
    while (ctx->current != NULL && (ctx->current->type == '&&')) {
      ...
      if (code != 0) {
        ...
        compiler_set_error_with_token(C_INVALID_TYPE, ctx, ctx->current,
                                      "invalid types logicl and expression");
        throw;
      }
      p_expression_node = move new_expression;
    }
  ...
  */
  ...
  try {
    ...
    while (ctx->current != NULL && (ctx->current->type == '||')) {
      ...
      if (!type_is_scalar(&new_expression->left->type)) {
        compiler_set_error_with_token(
            C_LEFT_IS_NOT_SCALAR, ctx, ctx->current,
            "left type is not scalar for or expression");
        throw;
      }

      if (!type_is_scalar(&new_expression->right->type)) {
        compiler_set_error_with_token(
            C_RIGHT_IS_NOT_SCALAR, ctx, ctx->current,
            "right type is not scalar for or expression");
        throw;
      }
  ...
  try {
    ...
    while (ctx->current != NULL &&
      ...
      if (category == TYPE_CATEGORY_FUNCTION) {
        compiler_set_error_with_token(C_ASSIGNMENT_OF_FUNCTION, ctx,
                                      ctx->current, "assignment of function");
      } else if (category == TYPE_CATEGORY_ARRAY) {
        compiler_set_error_with_token(
            C_ASSIGNMENT_TO_EXPRESSION_WITH_ARRAY_TYPE, ctx, ctx->current,
            "assignment to expression with array type");
      } else {
        if (type_is_const(&new_expression->left->type)) {
          compiler_set_error_with_token(C_ASSIGNMENT_OF_READ_ONLY_OBJECT, ctx,
                                        ctx->current,
      ...
      if (type_is_lvalue(&new_expression->left->type)) {
        // TODO
        // compiler_set_error_with_token(C_LVALUE_ASSIGNMENT, ctx, ctx->current,
        // "lvalue assignment");
      }
      ...
      if (op_token->type == '=') {
        check_assigment(ctx, &new_expression->left->type, new_expression->right,
                        new_expression->move_assignment != NULL, false);
      }
      ...
      new_expression->type.storage_class_specifier_flags &=
          ~STORAGE_SPECIFIER_FUNCTION_RETURN_NODISCARD;

      check_diferent_enuns(ctx, op_token, new_expression->left,
                           new_expression->right);
  ...
  */
  ...
  try {
    ...
    if (ctx->current && ctx->current->type == '?') {
      struct expression *owner p_conditional_expression =
          calloc(1, sizeof *p_conditional_expression);
      ...
      if (constant_value_is_valid(
              &p_conditional_expression->condition_expr->constant_value)) {
        if (constant_value_to_bool(
                &p_conditional_expression->condition_expr->constant_value)) {
          /*this is an extensions.. in constant expression we can mix types!*/
          p_conditional_expression->type =
              move type_dup(&p_conditional_expression->left->type);
          p_conditional_expression->constant_value =
              p_conditional_expression->left->constant_value;
          goto continuation;
        } else {
          /*this is an extensions.. in constant expression we can mix types!*/
          p_conditional_expression->type =
              move type_dup(&p_conditional_expression->right->type);
          p_conditional_expression->constant_value =
              p_conditional_expression->right->constant_value;
          goto continuation;
        }
      }
      ...
      if (expression_is_subjected_to_lvalue_conversion(
        ...
        left_type =
            move type_lvalue_conversion(&p_conditional_expression->left->type);
      } else {
        left_type = move type_dup(&p_conditional_expression->left->type);
      }

      if (expression_is_subjected_to_lvalue_conversion(
              p_conditional_expression->right)) {
        right_type =
            move type_lvalue_conversion(&p_conditional_expression->right->type);
      } else {
      ...
      if (!type_is_scalar(&p_conditional_expression->condition_expr->type)) {
        compiler_set_error_with_token(C_CONDITION_MUST_HAVE_SCALAR_TYPE, ctx,
                                      ctx->current,
      ...
      if (type_is_struct_or_union(&left_type) !=
        ...
        if (!type_is_same(&left_type, &right_type, true)) {
          compiler_set_error_with_token(
              C_INCOMPATIBLE_TYPES, ctx,
              p_conditional_expression->condition_expr->first_token,
              "incompatible types");
        }
      ...
      /*
         ...
         has type nullptr_t; or — one operand is a pointer to an object type and
      ...
      */
      if (type_is_pointer(&left_type)) {
        if (is_null_pointer_constant(p_conditional_expression->right) ||
            type_is_nullptr_t(&right_type) || type_is_void_ptr(&right_type)) {
        ...
        if (type_is_pointer(&right_type)) {
          ...
          if (!type_is_same(&left_type, &right_type, false)) {
            ...
            compiler_set_error_with_token(C_INCOMPATIBLE_TYPES, ctx,
                                          ctx->current, "incompatible types");
            goto continuation;
          }
          type_swap(&p_conditional_expression->type, &right_type);
          goto continuation;
      ...
      if (type_is_pointer(&right_type)) {
        if (is_null_pointer_constant(p_conditional_expression->left) ||
            type_is_nullptr_t(&left_type) || type_is_void_ptr(&left_type)) {
          type_swap(&p_conditional_expression->type, &right_type);
          goto continuation; // ok
        }
...
struct expression *owner constant_expression(struct parser_ctx *ctx) {
  ...
  if (p_expression && !constant_value_is_valid(&p_expression->constant_value)) {
    compiler_set_error_with_token(C_EXPECTED_CONSTANT_EXPRESSION, ctx,
                                  ctx->current, "expected constant expression");
  }
 ...
 */
bool expression_is_subjected_to_lvalue_conversion(
  ...
  if (expression->type.storage_class_specifier_flags &
